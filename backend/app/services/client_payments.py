...
def _prodamus_paid_content(title: Optional[str], access_url: Optional[str]) -> str:
    """Поле «Доступы к материалам» (оно же «Платный контент») у Продамуса.

    ⚠️⚠️ БЕЗ НЕГО ПРОДАМУС НЕ ПРОБИВАЕТ ЧЕК. Их формулировка: «Чтобы мы
    пробили чек, важно заполнить поле "доступы к материалам", которые получит
    пользователь после оплаты, иначе согласно 54-ФЗ необходимо пробить чек с
    помощью собственной онлайн-кассы». То есть без этого поля фискализация
    ложится на клиента — а у него своей кассы нет.

    ⚠️ Это НЕ название товара. Название — отдельное поле `products[0][name]`,
    туда эту инструкцию пихать нельзя (пробовали 17.09.2026, откачено).

    Что кладём: куда человек попадёт за материалами. Ссылка у каждого типа
    заказа своя — её передаёт вызывающий (`access_url`): у продукта страница
    материалов, у события кабинет участника. Ссылки нет — остаётся текст без
    неё: поле всё равно должно быть заполнено.

    Лимит поля у Продамуса — 4096 символов, наш текст в него укладывается
    с огромным запасом, резать нечего.
    """
    name = (title or "Участие в событии").strip()
    text = (f'Доступ : "{name}". После оплаты вы будете переброшены '
            f'на страницу со ссылками и получите письмо')
    access_url = (access_url or "").strip()
    if access_url:
        text = f"{text}. Ссылка на доступ: {access_url}"
    return text[:4096]


def _order_ref(order_id, title: Optional[str] = None) -> str:
    """Номер заказа для платёжной системы — только префикс и число.
    ...
    такой номер не узнавал ни один разбор в вебхуке, то есть оплата продукта
    не отмечалась бы вовсе. Ради этого функция и заведена.

    ⚠️⚠️ НАЗВАНИЕ ТАРИФА СЮДА НЕ ДОПИСЫВАТЬ. Пробовали 17.09.2026: Продамус
    показывает `order_id` в шапке («Оплата заказа №48861971 (evt-188)»), и
    туда заманчиво дописать название — состав заказа он на странице оплаты
    не выводит вовсе. Но с длинным номером его страница ВИСИТ ~35 секунд и
    отдаёт редирект в никуда: «Превышено время ожидания ответа». Параметр
    `title` оставлен в сигнатуре, чтобы вызовы не переписывать, и НЕ
...
def _prodamus_link(
    *,
    ...
    params: dict = {
        ...
        "products": [{
            # ⚠️ Название для ЧЕКА, а не просто имя тарифа (17.09.2026).
            # Продамус просит, чтобы из названия было понятно, что человек
            # покупает, — иначе в чеке остаётся невнятное «Тариф 3».
            # Формат задан владельцем: «Доступ : "Название". После оплаты…».
            # Хвост с инструкцией сохраняем целиком, режем только название —
            # иначе обрежется как раз то, ради чего текст и писался.
        ...
        # ⚠️⚠️ БЕЗ ЭТОГО ПОЛЯ ПРОДАМУС НЕ ПРОБИВАЕТ ЧЕК — фискализация
        # ложится на клиента, а своей онлайн-кассы у него нет. Подробности
        # и формулировка — в `_prodamus_paid_content`.
        # Ссылка на доступ — это `urlSuccess`: ровно та страница, куда
        # человек попадает после оплаты (у события — кабинет участника,
        # у продукта — страница с материалами).
        "paid_content": _prodamus_paid_content(title, redirect_url_ok),

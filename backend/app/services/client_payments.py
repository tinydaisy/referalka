...
#: Хвост названия товара в чеке Продамуса — что будет после оплаты.
#: Формулировка владельца (17.09.2026), менять только по его просьбе.
PRODAMUS_NAME_TAIL = ("После оплаты вы будете переброшены на страницу "
                      "со ссылками и получите письмо")
#: Сколько символов оставляем названию события/тарифа. Хвост длинный, и без
#: ограничения строка у длинных названий раздувается; режем название, а не
#: хвост — инструкция покупателю должна дойти целиком.
PRODAMUS_TITLE_LIMIT = 60


def _prodamus_product_name(title: Optional[str]) -> str:
    """Название товара для Продамуса: «Доступ : "X". После оплаты…».

    ⚠️ Только для Продамуса. У LeadPay и Т-Банка название собирается своими
    правилами (и свои лимиты) — общей функции тут быть не должно.
    """
    name = (title or "Участие в событии").strip()
    if len(name) > PRODAMUS_TITLE_LIMIT:
        name = name[:PRODAMUS_TITLE_LIMIT - 1].rstrip() + "…"
    return f'Доступ : "{name}". {PRODAMUS_NAME_TAIL}'
    ...
    *,
    ...
    params: dict = {
        ...
        "products": [{
            ...
            "name": _prodamus_product_name(title),
            "price": amount,
            "quantity": "1",
        }],

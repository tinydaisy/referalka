...
def validate_owner_username(username: str) -> Optional[str]:
    """Проверка ПУБЛИЧНОГО ника человека (владельца бота, службы заботы).

    ⚠️⚠️ БЕЗ ЮЗЕРНЕЙМА УСЛУГА НЕВОЗМОЖНА В ПРИНЦИПЕ. BotFather понимает только
    @username: и передача владения (`/setowner`), и назначение админом идут по
    нику — числовой id он не принимает. Значит аккаунт без публичного имени
    пользователя пройти автонастройку не может, и узнать об этом человек должен
    ДО постановки в очередь, а не после оплаты на предпоследнем шаге.

    ⚠️ Правила Telegram для имени пользователя мягче, чем для бота: 5..32
    символа, латиница/цифры/`_`, начинается с буквы. Окончание на `bot`
    наоборот НЕ требуется (и для человека невозможно).
    """
    u = (username or "").strip().lstrip("@").rstrip("/").split("/")[-1]
    if not u:
        return "Укажите ваш ник в Telegram"
    # Ссылку-приглашение (`+AbC…`) в качестве ника не принимаем: это не имя
    # пользователя, и передать по ней права нельзя.
    if u.startswith("+"):
        return ("Нужен публичный ник (@имя), а не ссылка-приглашение — "
                "по ней Telegram передать права не даёт")
    if not re.fullmatch(r"[A-Za-z][A-Za-z0-9_]{4,31}", u):
        return ("Ник может состоять из латинских букв, цифр и знака _, "
                "начинаться с буквы, длина 5–32 символа. "
                "Завести: Telegram → «Настройки» → «Имя пользователя»")
    return None
...
async def check_health(acc: SetupAccount) -> Health:
    ...
    try:
        ...
        # ⚠️ 500 символов обрезали ответ на самом важном месте: русский текст
        # про ограничение занимает ~480 символов, и дата снятия («Ограничения
        # будут сняты 20 Sep 2026…») не помещалась — а это ровно то, что нужно
        # знать. 2000 хватает с запасом любому ответу @SpamBot.
        out.note = (r or "").strip()[:2000]
        low = out.note.lower()
        ...
        # ⚠️⚠️ СВЕРЯЕМ ПО КОРНЮ, БЕЗ ХВОСТА (18.09.2026). Здесь стоял маркер
        # «ограничен » С ПРОБЕЛОМ на конце, а Telegram пишет «Ваш аккаунт
        # временно ограничен:» — с ДВОЕТОЧИЕМ. Не совпадало ни одного маркера,
        # ни хорошего ни плохого, и живой ответ уходил в `unknown`. Очередь
        # берёт только `ok` — аккаунт 998700388272 молча выбыл из работы, а
        # админка показывала «Не проверялся», хотя проверка исправно шла
        # каждые 6 часов.
        #
        # ⚠️ Пробел был не случайным: он защищал от слова «ограничений» в
        # ХОРОШЕЙ фразе («свободен от каких-либо ограничений»). Защита
        # осталась, но теперь она в порядке проверки — OK_MARKERS сверяются
        # ПЕРВЫМИ и выходят раньше (см. ниже), поэтому корень безопасен.
        BAD_MARKERS = (
            "limited", "restricted",                # английский
            "cheklangan", "cheklandi",              # узбекский: «ограничен»
            "ограничен", "ограничения применены",   # русский, по корню
        )

...
def _wrap_html_body(inner_html: str) -> str:
    ...
    Контент лежит на светло-голубой плашке (#E8F2FA) — фирменный стиль писем
    ПЛЮСОНа; подвал отписки ставится ниже, на белом.

    ⚠️ Вёрстку держать в синхроне с `build_email_body` (email_body.py) — это
    два входа в одно и то же письмо (готовый HTML / автогенерация из plain).
    """
    return (

...
def normalize_phone(phone: Optional[str]) -> Optional[str]:
    ...
    ⚠️ ТОЛЬКО ЦИФРЫ, без плюса: нормализованный номер — это не то, что
    показывают человеку, а ключ поиска. Плюс, пробелы, скобки и дефисы к
    самому номеру отношения не имеют и лишь плодят разные написания одного
    и того же: `+79161234567` и `79161234567` для базы были разными строками,
    и дубли не склеивались.

    Российский (8… / 9… / 7…) → `7XXXXXXXXXX`. Остальные — как есть, цифрами.
    Ведущие нули набора («+007…», «+0037…») срезаются.

    Человеку по-прежнему показывается `contacts.phone` в том виде, как он его
    ввёл — это поле нормализация не трогает.
    """
    if not phone:
        return None
    ...
    if len(digits) == 11 and digits.startswith('7'):
        return digits
    # Ведущие нули — мусор набора, к номеру не относятся.

...
async def pdf_response(url: str, *, filename_base: str) -> Response:
    ...
    ⚠️ Добавляем `pdf=1` ЗДЕСЬ, в одной точке на событие и продукт: по этому
    флагу страница отдаёт печатный вариант там, где на бумаге интерактив не
    работает (галерея-карусель показывает первую карточку и кнопку «Смотреть
    ещё» вместо ленты, которую в PDF никак не прокрутить).
    """
    url = f"{url}{'&' if '?' in url else '?'}pdf=1"
    try:

...
После миграции 033 bot_token живёт в channels (а не в clients), а флаг
...
async def get_client_telegram_token(client_id: int, db) -> Optional[str]:
    ...
    Если у клиента нет настроенного канала — возвращает None.
    """
...
async def upsert_client_telegram_token(client_id: int, bot_token: str, db) -> None:
    ...
    existing = await db.fetchval(
        "SELECT id FROM channels WHERE client_id = $1 AND platform = 'telegram' ORDER BY id LIMIT 1",
        client_id
    )
    if existing:
        await db.execute(
...
async def mark_unsubscribed_by_tg_id(client_id: int, tg_id: str, db) -> None:
    ...
    Создаёт строку в platform_user_channels если её ещё не было.
    """
    channel_id = await get_client_telegram_channel_id(client_id, db)
    if not channel_id:
        return
    await db.execute(
        """INSERT INTO platform_user_channels (platform_user_id, channel_id, is_unsubscribed, unsubscribed_at)
           SELECT pu.id, $1, TRUE, NOW()
             FROM platform_users pu
            WHERE pu.client_id = $2 AND pu.platform_user_id = $3

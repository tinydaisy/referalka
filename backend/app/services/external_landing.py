"""
Резолв партнёрского параметра внешней платформы клиента (collaborators.external_ref_param)
...
Связывает партнёра ПЛЮСОН (по ref_code) с партнёром во внешней системе клиента
...
async def resolve_external_ref_param(
    ...
    """Возвращает строку-параметр (например "gcpc=fdd97") коллаборатора по pid, либо None.

    Ищет contact с ref_code=pid (или в merged_ref_codes) у того же клиента, у которого
    к этому контакту привязан коллаборатор с непустым external_ref_param.

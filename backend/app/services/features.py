...
Slug-и фич (миграция 067; названия для клиента живут в `features.name` и
правятся ДАННЫМИ — здесь только смысл):
  channels         — свои чат-боты клиента в Telegram, MAX и ВКонтакте
  conference       — модуль Конференции
  awards           — модуль Премий (на вырост)
  lead_magnets     — лид-магниты + воронки выдачи
  export_contacts  — экспорт контактов в CSV

⚠️ Есть ОПИСАТЕЛЬНЫЕ фичи — они существуют только чтобы дать строку в карточке
тарифа, и `client_has_feature` по ним не вызывается: `miniapp_funnels`
(миграция 353), `referral_program` (355). Прежде чем закрывать ими что-то в
коде, убедиться, что это действительно решено сделать платным.

⚠️ Строку можно спрятать из карточки, не отбирая доступ — колонка
`features.hidden_in_card` (миграция 353).

...
def build_key(
    ...
    *,
    ...
    # Отзывы и кейсы — общая база КАБИНЕТА, к событию не привязаны.
    # ⚠️ Папка именно `cases`: по ней «Хранилище» узнаёт вид файла
    # (`if "/cases/" in key` в uploads.py). Назовёшь иначе — файл попадёт
    # в список без раздела и ссылки на «Отзывы и кейсы».
    if kind == "testimonial":
        return f"{base}/cases/{fname}"
    ...
    if kind == "cover_bg":
        ...
        # ⚠️ Здесь стояло `{name}` — переменной с таким именем в функции нет, и
        # загрузка фона обложки падала с NameError (во всей функции имя файла
        # зовётся `fname`). Ошибка тихая: до этой строки доходит только загрузка
        # фона обложки, остальные виды шли своими ветками.
        return f"{base}/covers/bg/{fname}"

    # Фон генератора афиш (миграция 435). ⚠️ В отличие от фона обложки — В ПАПКЕ
    # СОБЫТИЯ: макет афиши свой у каждой конференции, и при разборе хранилища
    # должно быть видно, к какому событию относится картинка.
    if kind == "poster_bg":
        if not event_id:
            raise ValueError("poster_bg требует event_id")
        return f"{base}/events/{event_id}/poster_bg/{fname}"

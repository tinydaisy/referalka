...
# ⚠️ У LeadPay цену принимает только v2 (`api/v2/getLink`): старый getLink брал
# её из карточки товара и переданную молча игнорировал — промокод пропадал без
# следа. Все боевые ссылки идут через v2, карточки не нужны (см. client_payments).
# ⚠️ Метод `rest/v3/salebot/link`, который тут раньше назывался, в коде НЕ
# используется — не искать его.
...
# Проверка
# ─────────────────────────────────────────────────────────────────────────────
def clamp_to_provider_minimum(price_after: int, provider: Optional[str]) -> int:
    """Поднять цену до минимума платёжной системы.

    ⚠️⚠️ У LeadPay минимальный платёж 100 ₽ — заказ дешевле их страница
    оплаты не примет вовсе. Поэтому скидку, сбивающую цену ниже порога,
    ОГРАНИЧИВАЕМ, а не отказываем в ней: человек уже ввёл код и ждёт скидку,
    и отказ на этом шаге читается как «промокод не работает». Так же
    поступает и сам LeadPay в своих промокодах.

    ⚠️ НОЛЬ НЕ ТРОГАЕМ: цена 0 означает «платёжная система не нужна вовсе» —
    доступ выдаётся сразу, до обращения к ней (см. event_orders.py). Поднять
    его до 100 ₽ значило бы взять деньги там, где скидка 100%.
    """
    if price_after <= 0:
        return price_after
    from app.services.client_payments import min_payment_rub
    return max(price_after, min_payment_rub(provider))

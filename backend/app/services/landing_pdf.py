...
# ⚠️ Потолок высоты листа. Значение проверено печатью на проде: Chrome
# принимает и 40000 px одним листом — прежние 18000 были перестраховкой «по
# документации», и из-за них длинные лендинги резались НАДВОЕ, причём разрыв
# приходился на середину заголовка (жалоба с телефона: «на заголовки что-то
# наезжает»). Держим с запасом, но не бесконечно: совсем гигантский лист —
# это уже риск получить пустой файл вместо ошибки.
_MAX_SHEET_PX = 60000
...
def _pdf_params(height_px: int) -> dict:
    """Размер листа = вся высота полотна, чтобы лендинг вышел ОДНИМ листом.

    ⚠️ Резать по высоте нельзя: разрыв приходится на середину заголовка или
    карточки, и на телефоне это читается как «съехала вёрстка» (именно так и
    выглядела жалоба клиента со скриншотом).
    """

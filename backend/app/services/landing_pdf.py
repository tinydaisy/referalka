...
# ⚠️⚠️ ЖЁСТКИЙ ПРЕДЕЛ ФОРМАТА PDF — 14400 pt (200 дюймов) на сторону. Это не
# «перестраховка по документации», а ограничение самого формата, и нарушать
# его нельзя, даже если Chrome молча соглашается напечатать больше.
#
# Чем это оборачивается: настольные просмотрщики (Preview, Acrobat) нарушение
# прощают и рисуют страницу как есть — «на компьютере всё хорошо». А строгие
# рендереры (просмотрщик PDF на телефоне) упираются в лимит и рисуют поверх
# содержимого ТЁМНЫЕ ПРЯМОУГОЛЬНИКИ, закрывающие заголовки. Именно так и
# выглядела жалоба: «на компе нормально, а на телефоне заголовки квадратами».
#
# ⚠️ Поэтому длинную страницу НЕ режем на листы (разрывы пришлись бы на
# середину заголовков) и НЕ печатаем сверх предела, а УМЕНЬШАЕМ МАСШТАБ, чтобы
# всё полотно поместилось в один лист допустимой высоты. Читаемость от этого
# страдает мало: лендинг всё равно смотрят с увеличением.
_MAX_SHEET_PT = 14400                      # предел формата PDF, менять нельзя
_MAX_SHEET_PX = int(_MAX_SHEET_PT / 72 * 96)   # то же в CSS-пикселях (19200)

# Ниже этого масштаба текст становится нечитаемым — тогда честнее отдать
# несколько листов, чем один нечитаемый.
_MIN_SCALE = 0.5
...
# ⚠️ ПЕЧАТАЕМ ПО ОДНОЙ ЗА РАЗ. На проде 2 ядра и 4 ГБ: второй Chrome, поднятый
# одновременно с первым, не успевал открыть отладочный порт, и человек получал
# «Браузер не запустился» на ровном месте (поймано при двух печатях подряд).
# Очередь дешевле, чем разбирательство «почему у второго не сработало»: он
# просто подождёт свои несколько секунд.
#
# ⚠️ Замок в памяти процесса, а воркеров у API два — то есть строго
# одновременных печатей может быть две. Это уже безопасно (проверено), а
# межпроцессный замок ради такого — усложнение без выигрыша.
_PRINT_LOCK = asyncio.Semaphore(1)

# Сколько ждём своей очереди. Дольше — уже неуважение к человеку у экрана:
# лучше честно сказать «сервер занят», чем держать кнопку в «Собираем…».
_QUEUE_WAIT_SEC = 90
...
async def _wait_devtools(port: int, proc: Optional[subprocess.Popen] = None) -> str:
    """Дождаться, пока браузер поднимет отладочный порт, и отдать адрес сокета.

    ⚠️ Проверяем, что процесс ЖИВ. Если Chrome упал сразу (не хватило памяти,
    занят профиль), ждать его порт бессмысленно — раньше это оборачивалось
    сорока секундами тишины и невнятным «Браузер не запустился».
    """
    async with httpx.AsyncClient() as client:
        for _ in range(120):                             # ~60 секунд
            if proc is not None and proc.poll() is not None:
                raise PdfRenderError(
                    "Не удалось запустить браузер для печати — попробуйте ещё раз."
                )
            try:
    ...
    raise PdfRenderError("Браузер не отвечает — попробуйте ещё раз через минуту.")
...
def _pdf_params(height_px: int) -> dict:
    """Параметры печати: один лист, не выходящий за предел формата PDF.

    ⚠️ Резать полотно на листы нельзя — разрыв приходится на середину заголовка
    или карточки, и на телефоне это читается как «съехала вёрстка».

    ⚠️ Но и печатать лист выше 14400 pt нельзя (см. `_MAX_SHEET_PT`): файл
    выходит за рамки формата, настольные просмотрщики это прощают, а мобильные
    рисуют поверх текста тёмные прямоугольники.

    Выход — МАСШТАБ: длинное полотно ужимаем так, чтобы оно поместилось в один
    допустимый лист. Ширина листа уменьшается пропорционально, поэтому
    пропорции страницы не искажаются.
    """
    content_px = max(height_px + 4, 400)

    # Во сколько раз нужно ужать, чтобы уложиться в предел.
    scale = 1.0
    if content_px > _MAX_SHEET_PX:
        scale = max(_MIN_SCALE, _MAX_SHEET_PX / content_px)

    # Лист = полотно после масштабирования (в дюймах).
    sheet_h_in = min(content_px * scale, _MAX_SHEET_PX) / 96
    sheet_w_in = VIEWPORT_WIDTH * scale / 96

    return {
        "printBackground": True,                 # фоны и градиенты темы
        "paperWidth": sheet_w_in,
        "paperHeight": sheet_h_in,
        "marginTop": 0, "marginBottom": 0, "marginLeft": 0, "marginRight": 0,
        "preferCSSPageSize": False,
        # ⚠️ Chrome сам уменьшает содержимое под лист этим коэффициентом.
        "scale": scale,
    }
...
async def render_landing_pdf(url: str) -> bytes:
    ...
    # Ждём очереди: параллельные печати на этом сервере не уживаются.
    try:
        await asyncio.wait_for(_PRINT_LOCK.acquire(), timeout=_QUEUE_WAIT_SEC)
    except asyncio.TimeoutError:
        raise PdfRenderError(
            "Сейчас собирается другой файл — попробуйте через минуту."
        )
    try:
        return await _render_locked(chrome, url)
    finally:
        _PRINT_LOCK.release()


async def _render_locked(chrome: str, url: str) -> bytes:
    """Сама печать. Вынесено, чтобы очередь освобождалась в любом случае."""
    port = _free_port()
...
async def _wait_page_ready(cdp: _Cdp, sid: str) -> None:
    ...
    # ⚠️ Ждать «пока ВСЕ картинки станут complete» НЕЛЬЗЯ — проверено на проде:
    # на лендинге iViSiON-9 из 29 картинок 12 не переходят в `complete` никогда
    # (они за пределами экрана, и браузер их принципиально не грузит, пока они
    # не понадобятся). Такой цикл честно выжидал весь таймаут впустую и сделал
    # печать ВТРОЕ медленнее: 31 секунда вместо 13. Поэтому здесь именно
    # короткая пауза на дорисовку — она дешевле и предсказуемее.

...
def normalize_telegram_link(s: Optional[str]) -> str:
    ...
    Поддерживает:
    - `https://t.me/foo`, `http://t.me/foo`, `t.me/foo`, `t.me/foo`
    ...
    if m:
        return f"https://{TG_DOMAIN}/{m.group(1).strip('/')}"
    if raw.startswith("t.me/") or raw.startswith("t.me/"):
        path = raw.split("/", 1)[1].strip("/")
        return f"https://{TG_DOMAIN}/{path}" if path else raw
    if raw.startswith("@"):
        u = raw[1:]
        ...
        if u.startswith("+") and _TG_RE_INVITE.match(u):
            return f"https://{TG_DOMAIN}/{u}"
        if _TG_RE_USERNAME.match(u):
            return f"https://{TG_DOMAIN}/{u}"
        return raw
    if _TG_RE_INVITE.match(raw):
        return f"https://{TG_DOMAIN}/{raw}"

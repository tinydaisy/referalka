...
async def resolve_landing_url(conn, event_id: int, *, client_id: int | None = None) -> str:
    ...
    ⚠️ Способ НЕ задан → простая форма регистрации. Раньше здесь угадывали
    «заполнен landing_url → сторонний», но у подавляющего большинства он
    пуст, а у кого заполнен — там нередко лежит ссылка на бота или на чужое
    событие, и человек с рассылки уезжал не туда. Дефолт должен быть тем,
    что работает у всех и всегда, а не тем, что случайно оказалось в поле.
    ...
    if mode == "form":
        return form_url
    # Режим не задан — простая форма (см. ⚠️ в докстроке).
    return form_url
...
def build_pre_start_message(tmpl_text, speaker_name, speaker_topic, stream_url_val,
    ...
    # отдельной ссылкой, чтобы его картинка попала в эту комнату.
    #
    # ⚠️ Ссылка стоит НА ОТДЕЛЬНОЙ СТРОКЕ под подписью «Ссылка для входа (Zoom):».
    # Обычное правило (убрать строку с плейсхолдером) оставило бы висеть саму
    # подпись — заголовок без ссылки под ним. Поэтому при пустом значении

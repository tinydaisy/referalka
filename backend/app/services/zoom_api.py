...
# Права, которые должны быть отмечены В САМОМ ПРИЛОЖЕНИИ Zoom (вкладка Scopes).
#
# ⚠️⚠️ ЗДЕСЬ СПРАВОЧНЫЙ СПИСОК — В ЗАПРОС ОН НЕ ПЕРЕДАЁТСЯ (см. `oauth_url`).
# Zoom берёт права из настроек приложения, а параметр `scope` в ссылке
# авторизации только сужает их и на нескольких значениях даёт ошибку 4700.
#
# ⚠️⚠️ ГРАНУЛЯРНЫЕ ИМЕНА, НЕ КЛАССИЧЕСКИЕ. Новые приложения (General App)
# получают только гранулярные scopes: `meeting:write` в таком приложении
# отвечает «Invalid scope» прямо на экране согласия — так и произошло при
# первом подключении 18.09.2026.
#
# ⚠️⚠️ `meeting:write:meeting` НЕ ПОКРЫВАЕТ LIVESTREAM. Гранулярные права не
# иерархичны: создать конференцию и настроить ей трансляцию — два РАЗНЫХ
# права. Это главная ловушка перехода со старого `meeting:write`, который
# покрывал оба. Без `meeting:update:livestream` конференция создастся, а
# вещание в нашу комнату молча не включится.
#
# meeting:write:meeting      — создать конференцию (POST /users/me/meetings)
# meeting:update:livestream  — настроить ей вещание (PATCH /meetings/{id}/livestream)
# meeting:delete:meeting     — удалить прежнюю при пересоздании
# user:read:user             — чей аккаунт и какой тариф (ниже Pro вещания нет)
ZOOM_SCOPES = [
...
def oauth_url(state: str) -> str:
    ...
    ⚠️⚠️ ПАРАМЕТР `scope` СЮДА НЕ ДОБАВЛЯТЬ. Zoom берёт права из настроек
    приложения (вкладка Scopes); переданный в ссылке `scope` может их только
    сузить, обязан быть подмножеством отмеченного, а на нескольких значениях
    Zoom отвечает ошибкой 4700. Во всех официальных примерах Zoom этого
    параметра нет. Список нужных прав — в `ZOOM_SCOPES` выше, он справочный:
    отмечается руками в приложении. Что реально выдано — видно в поле `scope`
    ответа токен-эндпоинта.
    """

...
⚠️⚠️ ЛЕГАСИ. `process_conversion` и `get_participant_stats` НЕРАБОТОСПОСОБНЫ:
они пишут в `event_participants.points_total`, а такой колонки в базе нет.
Любой вызов упадёт 500. За 30 дней на проде не вызывались ни разу — механика
подарков давно живёт на порогах реф-программы (`event_referral_thresholds`),
а не на баллах участника.

Оставлены как есть до отдельного решения: удалять эндпоинт
`POST /api/v1/referral/conversion` наружу нельзя без проверки, не зовёт ли его
чей-нибудь сторонний лендинг. Новую логику сюда НЕ добавлять.

Рабочая функция здесь одна — generate_ref_code().
"""
import secrets
import string
...
async def process_conversion(
    ...
    """
    # Ищем участника и его событие (ref_code живёт в contacts с миграции 036)
    # Fallback: ищем в merged_ref_codes для исторических связей после ручного мерджа

...
async def assistant_permission_guard_middleware(request: Request, call_next):
    ...
    # ⚠️⚠️ КАБИНЕТ ВНЕДРЕНЦА — НЕ ЗОНА ЭТОГО ОХРАННИКА (23.09.2026).
    # Один и тот же человек бывает и помощником в чьём-то кабинете, и
    # внедренцем: это РАЗНЫЕ роли одного человека, и здесь они пересекаться не
    # должны. Токен лежит в браузере под общим ключом, поэтому на `/tech/*`
    # запросто приезжает ассистентский пропуск от прошлого входа — и охранник
    # отвечал «У вас доступ только к своим контактам и CRM событий», хотя
    # человек шёл в кабинет внедренца и про контакты не спрашивал. Вход
    # выглядел сломанным: четыре менеджера лидов не могли войти вовсе.
    #
    # Пропускаем, а не разрешаем: за `/api/v1/tech/*` отвечает своя проверка
    # `get_current_tech` — она требует роль `tech` и вернёт 401, если пропуск
    # чужой. Фронт по 401 отправит на форму входа внедренца, что и нужно.
    # Перечислять эти пути в списках уровней нельзя: тогда помощник получил бы
    # доступ к кабинету внедренца по ассистентскому пропуску.
    if path.startswith("/api/v1/tech/") or path == "/api/v1/tech":
        return await call_next(request)

...
async def save_requisites(
    ...
    # ⚠️ Отметка ОБЯЗАТЕЛЬНА (владелец, 26.09.2026): выплаты — только
    # самозанятым. Проверяем здесь, а не только кнопкой: запрос можно
    # отправить и мимо экрана.
    if not data.self_employed:
        raise HTTPException(400, "Отметьте, что вы самозанятый — без этого "
                                 "выплаты невозможны")

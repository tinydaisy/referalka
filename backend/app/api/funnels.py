...
async def _landing(slug: str, kind: str, request: Request) -> RedirectResponse:
    ...
    async with pool.acquire() as db:
        ...
        # Создаём funnel_run со stage=landed (нет привязки к платформе/пользователю —
        # эти поля заполнятся когда бот получит /start)
        run_id = await db.fetchval(
            """INSERT INTO funnel_runs
                  (client_id, type, lead_magnet_id, package_id,
                   referrer_contact_id, utm, stage, landed_at)
               VALUES ($1, 'lead_magnet', $2, $3, $4, $5::jsonb, 'landed', NOW())
               RETURNING id""",

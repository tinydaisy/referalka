...
действий клиента (зайти в бота, вступить в группу). Не забрал за 1,5 суток — бот
...
async def get_state(user=Depends(get_current_client), db=Depends(get_db)):
    ...
    return {
        ...
        # ⚠️⚠️ БЕРЁМ ИЗ КОНСТАНТЫ ЗАДАЧИ, А НЕ ЧИСЛОМ ЗДЕСЬ. Тройка была
        # зашита руками: срок поменяли на 1,5 суток в `CLAIM_DAYS`, а экран
        # продолжал бы обещать три дня — человек читал бы одно, а получал
        # другое. Одно значение на весь проект.
        #
        # ⚠️ Импорт ВНУТРИ функции: модуль задач тянет celery, и на уровне
        # файла это замедлило бы старт API ради одного числа.
        "claim_days": __import__(
            "app.tasks.tg_setup", fromlist=["CLAIM_DAYS"]).CLAIM_DAYS,
        ...
        # бот сгорит через 1,5 суток, заняв слот. Поэтому запуск требует
...
async def transfer_now(user=Depends(get_current_client), db=Depends(get_db)):
    ...
    if order["bot_transferred_at"] and order["group_transferred_at"]:
        return {"ok": True, "already": True}
    # ⚠️⚠️ ОТКАЗА «СНАЧАЛА ЗАЙДИТЕ В БОТА» ЗДЕСЬ БОЛЬШЕ НЕТ (18.09.2026).
    #
    # Он стоял на входе и не пускал дальше, если в базе нет отметки
    # `client_started_bot_at`. Но отметка ставится ботом асинхронно и её может
    # не быть, когда человек уже всё сделал: зашёл, нажал «Запустить» — а
    # кнопка «Доделать настройку» отвечает ему «сначала зайдите в бота».
    # Тупик на ровном месте, причём на экране, который для того и сделан,
    # чтобы из тупика выходить.
    #
    # ⚠️ Проверка не нужна и по сути: само требование Telegram («получатель
    # должен был написать боту») никуда не делось — его применяет BotFather в
    # момент передачи, и `transfer_bot` вернёт понятную ошибку с предложением
    # повторить. То есть мы дублировали чужую проверку своим догадочным
    # условием и на нём же спотыкались. К правам на ГРУППУ вход в бота и вовсе
    # отношения не имеет.
...
async def start_setup(data: StartRequest,
    ...
    # адрес = звать некуда: бот повисит 1,5 суток, займёт слот и сгорит.
    #

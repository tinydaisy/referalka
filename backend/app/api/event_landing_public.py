    ...
    if "partners" in kinds:
        rows = await db.fetch(
            f"""SELECT cse.id, cse.role, cse.partner_url,
                      ...
                      c.last_name,
                      -- ⚠️ Явная галочка «компания» (миграция 425): по ней
                      -- страница выбирает вид карточки. Раньше это выводили
                      -- из заполненной фамилии, и логотип партнёра-компании
                      -- уезжал в портретный квадрат.
                      c.is_company,
                      -- ⚠️ ВТОРОЙ логотип компании — «для светлого фона»
                      -- (заполняется в карточке человека, поле
                      -- `logo_on_light_url`). Какой из двух показать, решает
                      -- настройка СЕКЦИИ `partner_logo_variant`: основной
                      -- логотип часто светлый и на белой плашке пропадал.
                      c.logo_on_light_url,

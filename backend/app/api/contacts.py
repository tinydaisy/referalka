...
async def get_contacts(
    ...
    # кто написал одно только что, — а важнее свежесть. EXISTS вдобавок
    # останавливается на первой строке, в отличие от COUNT(*), который
    # пересчитывает всю переписку.
    #
    # ⚠️ Второй ключ — время последнего сообщения ПЕРЕПИСКИ, а не
    # `c.last_contact_at`. Это разные вещи: last_contact_at отмечает, когда
    # человек последний раз касался системы (зашёл, зарегистрировался), и
    # ответы оператора его не двигают. Из-за этого список вёл себя неверно:
    # контакт с перепиской сегодня стоял ниже того, чья переписка месячной
    # давности, а отвеченный диалог не опускался вниз и висел наверху.

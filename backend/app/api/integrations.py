...
async def _mark_payment(data: PaymentPaidRequest, db: asyncpg.Connection, *, status: str = "paid") -> dict:
    ...
    # ⚠️ СУММА ПИШЕТСЯ ТОЛЬКО ТА, ЧТО ПРИСЛАЛИ. Своей подстановки цены тарифа
    # здесь нет и быть не должно (решение владельца 09.09.2026): суммы вносятся
    # в данные явно, а не досчитываются кодом в момент записи. Внешняя система
    # присылает `amount` не всегда — такие записи заполняются разово, скриптом,
    # с проверкой по источнику оплаты.
    amount = data.amount

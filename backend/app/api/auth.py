...
async def referrer_info(
    ...
    return {
        ...
        # База нужна фронту, чтобы показать «30 дней вместо 7»: без второй цифры
        # непонятно, в чём выгода реф-ссылки. Триал даётся ОДИН раз, при создании
        # кабинета, — поэтому фронт подписывает «для новых пользователей»,
        # иначе люди считают, что переход по ссылке продлевает им триал снова.
        "base_days": int(base_days) + int(promo_bonus),
    }

...
class UpdateEventRequest(BaseModel):
    ...
    # Коллаб-событие: раздавать ли почту и телефон зарегистрировавшегося ВСЕМ
    # организаторам (миграция 371). FALSE — контакт остаётся только у того, кто
    # привёл. Партнёры договариваются об этом по-разному, решать за них нельзя.
    collab_share_contacts: Optional[bool] = None
    ...
    # ⚠️ ID вводится ВРУЧНУЮ (командой /getmyid в самом чате), а не выбором из
    # базы чатов клиента: служебный чат заводят под событие, в общую базу
    # рассылок ему попадать незачем. URL — только для справки, в отправке не
    # участвует (чтобы из кабинета быстро открыть нужный чат).
...
async def list_events(
    ...
    base_select = """
        SELECT e.id, e.slug, e.title, e.module_slug, e.status, e.is_collab,
               ...
               -- ⚠️ Нужен шапке события: по нему решается, предупреждать ли
               -- про отсутствующую дату перед публикацией. Без поля фронт
               -- ругался бы и на бессрочные события, где даты нет законно.
               e.is_evergreen,
...
_CHAT_SUBQ = """
    ...
    -- Чат спикеров (миграция 437): ID введён вручную, лежит прямо в events.
...
async def get_event_share_links_by_slug(
    ...
    # ⚠️ Веб-страницы без мессенджера ('web' и 'landing') — по запросу, как и
    # выше. Зовёт карточка спикера в дашборде: там партнёр видит полный набор
    # того, что может раздать.
    with_web: bool = False,
    db: asyncpg.Connection = Depends(get_db),
):
    ...
    links = await build_share_links(
        ...
        include_disabled=all_platforms, with_web=with_web,
    )
...
async def get_event_share_links(
    ...
    mode: str | None = None,
    cid: int | None = None,
    # ⚠️ Веб-страницы без мессенджера ('web' и 'landing') — по запросу. По
    # умолчанию выключены: ручку зовёт и Mini App, где набор ждут площадочный.
    with_web: bool = False,
    db: asyncpg.Connection = Depends(get_db),
):
    ...
    links = await build_share_links(
        ...
        link_mode=lm, include_disabled=True, with_web=with_web,
    )
...
async def update_event(
    ...
    if data.model_fields_set and data.status == "published" and event["status"] != "published":
        ...
        if not has_dates:
            # ⚠️ Про галочку «Идёт постоянно» говорим ПРЯМО В ОШИБКЕ. Она
            # сохраняется кнопкой «Сохранить» на вкладке «Основное», а кнопка
            # публикации живёт в шапке — человек ставит галочку, сразу жмёт
            # «Опубликовать» и получает отказ, хотя дата ему не нужна вовсе.
            # Прежний текст «укажите дату» отправлял его искать несуществующую
            # проблему вместо того, чтобы сохранить форму.
            raise HTTPException(
                status_code=409,
                detail="Укажите дату события — без неё оно не попадёт в календарь "
                       "и не будет напоминаний участникам. "
                       "А если событие идёт постоянно и даты у него нет — "
                       "поставьте галочку «Идёт постоянно» на вкладке «Основное» "
                       "и нажмите «Сохранить», потом публикуйте.",
            )
...
async def copy_event(
    ...
    async with db.transaction():
        ...
        new_event = await db.fetchrow(
            ...
            src.get('tg_speakers_chat_id'), src.get('vk_speakers_chat_id'),
            src.get('max_speakers_chat_id'),
        )
        new_id = new_event['id']
...
async def event_crm(
    ...
    # ⚠️ Роль внедренца нужна ДО сборки запроса: от неё зависят блоки ПЛЮСОНА.
    spec_id = await tech_spec_id_of(client)

    # ⚠️⚠️ БЛОКИ ПЛЮСОНА ВИДИТ И ВЛАДЕЛЕЦ КАБИНЕТА С ФИЧЕЙ (23.09.2026).
    # Раньше их видел только внедренец, и владелец платформенного кабинета не
    # мог посмотреть, кто из участников его событий дошёл до ПЛЮСОНа.
    #
    # ⚠️ Гейт ПО ФИЧЕ `plusson_crm_link` (миграция 499), а НЕ по `client_id IN
    # (1, 3)`: список id ломается молча — появится третий такой кабинет, и про
    # него никто не вспомнит, а найти этот список в коде по смыслу невозможно.
    # Фича выдана тарифу «Администратор», на котором сидят кабинеты платформы.
    show_plusson = bool(spec_id)
    if not show_plusson and not leads_gid:
        from app.services.features import client_has_feature
        show_plusson = await client_has_feature(db, client_id, "plusson_crm_link")

    # Обязательные параметры по порядку: $1 event_id, $2 client_id,
    # $3 дата события (для «был в ПЛЮСОНе до него»).
    # ⚠️ Свои номера заняли ЗДЕСЬ, до необязательных: номера ниже раздаёт
    # `len(args)`, и вставка в середину сдвинула бы их молча.
    extra = ""
    args: list = [event_id, client_id, ev["created_at"]]
    if leads_gid:
        args.append(leads_gid)
        extra += f"""
    ...
    rows = await db.fetch(
        f"""SELECT ep.id, c.id AS contact_id, c.name,
                  ...
                  --
                  -- ⚠️ Признака «этого привёл я сам» здесь НЕТ (решение
                  -- владельца 22.09.2026). Реферовод — КОНТАКТ в базе клиента,
                  -- а внедренец — КЛИЕНТ ПЛАТФОРМЫ: сущности разного уровня, и
                  -- совпадают они лишь случайно. У кабинета, не связанного с
                  -- ПЛЮСОНом, такого пересечения нет вовсе. «Покажи моих» уже
                  -- решено закреплением: менеджер лидов видит только своих.
    ...
    # ── Блоки ПЛЮСОНА: внедренцу и владельцу с фичей ─────────────────────
    ...
    # `show_plusson` вычислен выше — он нужен был ещё до сборки запроса.
    plusson_columns = []
    if show_plusson:
        plusson_groups = {
...
async def event_participants(
    ...
    rows = await db.fetch(
        f"""SELECT ep.id,
                  ...
                  -- ⚠️⚠️ Считаем ТОЛЬКО по `amount`: подставлять цену тарифа
                  -- вместо пустой суммы нельзя — вручную отмечают и тех, кто
                  -- прошёл бесплатно, и им приписались бы чужие деньги.

...
async def connect_telegram_bot(
    ...
    # ⚠️ Меню команд. Без `setMyCommands` человек не знает, что боту можно
    # писать команды: списка в интерфейсе Telegram нет, пока его не задали.
    # Обе команды в боте УЖЕ работают (`/app`, `/support` в
    # bot/handlers/start.py) — здесь мы только показываем их в меню. Ставить
    # команду, которой нет в коде, нельзя: человек нажмёт, а бот промолчит.
    # ⚠️ Тот же список, что в автонастройке (tasks/tg_setup.py): у ботов,
    # подключённых разными способами, меню не должно отличаться.
    await _tg_call(token, "setMyCommands", {
        "commands": [
...
async def instagram_oauth_callback(
    ...
    if not pages:
        ...
        return RedirectResponse(
            ...
            # выдано разрешение `pages_show_list`: страницы у человека есть,
            # но мы их не видим. Прежний текст отправлял чинить исправное —
            # человек шёл создавать вторую страницу вместо настройки доступа.
            f"{back}?ig_error=" + _up.quote(
                "Не удалось получить список ваших страниц Facebook. Обычно это значит, "
                "что при входе не были выданы права на страницы — попробуйте подключить заново "
                "и отметьте все галочки. Если не поможет — напишите нам, проверим настройки."
            ),
            status_code=302,
        )
    ...
    if not found:
        ...
        # ⚠️ Называем страницы ПОИМЁННО. Без списка человек не понимает, где
        # смотреть: у него страниц несколько, а сообщение говорит «ни к одной»
        # — и он идёт проверять не ту. С именами сразу видно, что мы нашли.
        _names = ", ".join(f"«{p.get('name') or p.get('id')}»" for p in pages[:5])
        # ⚠️ Показываем ВЫДАННЫЕ ПРАВА прямо в сообщении. Причин у этой ошибки
        # две и они противоположны: связки действительно нет — или её не видно,
        # потому что не выдан `instagram_basic`. Без списка прав человек чинит
        # не то: идёт перепривязывать исправный аккаунт.
        try:
            _p = await ig.graph_get("me/permissions", token=long_token)
            _granted = sorted(x.get("permission") for x in (_p.get("data") or [])
                              if x.get("status") == "granted")
        except Exception:
            _granted = []
        _no_basic = "instagram_basic" not in _granted
        _hint = (
            "Приложению не выдано право instagram_basic — без него связку "
            "со страницей не видно. Отзовите доступ (Facebook → Настройки → "
            "«Приложения и сайты» → PLUSON → Удалить) и подключите заново."
            if _no_basic else
            "Откройте приложение Instagram → Настройки → «Учётный аккаунт» → "
            "«Центр аккаунтов» и проверьте, какая страница связана с аккаунтом."
        )
        _ig_log.warning("Instagram OAuth: выданные права: %s", _granted)
        return RedirectResponse(
            f"{back}?ig_error=" + _up.quote(
                f"Нашли ваши страницы Facebook ({_names}), но Instagram не привязан "
                f"ни к одной из них. {_hint} "
                f"Выданные права: {', '.join(_granted) or 'не удалось получить'}."
            ),
            status_code=302,
        )

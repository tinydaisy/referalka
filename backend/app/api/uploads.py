...
IMAGE_UPLOAD_KINDS = {
    ...
    "survey_media",
    # ⚠️ Фото отзывов и кейсов. event_id НЕ требуют: отзывы — общая база
    # КАБИНЕТА, а не событие (их отбирают в галереи по тегам).
    # Раньше раздел грузил их видом `landing_media`, который требует event_id, —
    # и загрузка падала с 400 «landing_media требует event_id». Сам вид
    # `testimonial` при этом уже знали три места в «Хранилище», но в списке
    # разрешённых его не было.
    "testimonial",
    # Картинка новости платформы (миграция 374). ⚠️ event_id не требует и
...
async def storage_usage(
    ...
    # ⚠️ У клиента со СВОИМ хранилищем объём определяется его тарифом в Cloud.ru
    # (бесплатно 15 ГБ), а не нашей квотой — она к его файлам отношения не имеет.
    if own:
        quota = 15 * 1024 ** 3
    return {
        "used_bytes": used,

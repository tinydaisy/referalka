...
async def submit_survey(
    ...
    if not data.contact_id and not data.chosen_contact_id:
        ...
        # ⚠️ Не меньше 10 цифр: короче настоящего номера не бывает. Люди
        # вписывают «000» и «123» вместо телефона — такие записи потом не
        # склеиваются с человеком и мусорят базу (на проде таких уже есть).
        if len(re.sub(r"\D", "", phone)) < 10:
            raise HTTPException(400, "Проверьте телефон — кажется, номер неполный")

...
DEFAULT_STEPS = [
    ...
    },
    {
        ...
        "text": (
            ...
            "Возможно были сомнения? Если что-то осталось непонятным — напишите нам прямо в этом боте.\n\n"
            "А если всё хорошо — забронируйте место одним нажатием ↓"
        ),
        "button_label": "Хочу участвовать",
    },
    {
        "offset_minutes": 60 * 48,
        "text": (
    ...
    for i, step in enumerate(DEFAULT_STEPS):
        await db.execute(
            """INSERT INTO event_nurture_steps
                  (event_id, sort_order, offset_minutes, text, button_label, is_active)
               VALUES ($1, $2, $3, $4, $5, TRUE)""",
            event_id, i, step["offset_minutes"], step["text"], step["button_label"],
        )
...
async def list_nurture_steps(
    ...
    rows = await db.fetch(
        """SELECT id, sort_order, offset_minutes, text, button_label, is_active, updated_at
...
async def create_nurture_step(
    ...
    new_id = await db.fetchval(
        """INSERT INTO event_nurture_steps
           ...
           VALUES ($1, $2, $3, $4, $5, $6)
        RETURNING id""",
        event_id, next_sort, data.offset_minutes, data.text, data.button_label, data.is_active,
    )
    return {"id": new_id}
...
async def update_nurture_step(
    ...
    if data.offset_minutes is not None:
        args.append(data.offset_minutes); updates.append(f"offset_minutes = ${len(args)}")

...
async def create_assistant(
    ...
    # Почта клиента ПЛЮСОНа помощником быть не может: при входе система не поймёт,
    # чей пароль проверять — клиентский или помощника.
    if await db.fetchval("SELECT 1 FROM clients WHERE LOWER(email) = $1", email_norm):
        raise HTTPException(
            status_code=409,
            detail="Этот email зарегистрирован как кабинет ПЛЮСОНа. "
                   "Попросите помощника завести отдельный адрес.",
        )

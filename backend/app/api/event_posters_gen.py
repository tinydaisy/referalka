...
async def _sessions(db: asyncpg.Connection, event_id: int) -> dict:
    ...
    for r in rows:
        ...
        cur = out.setdefault(key, {
            # ⚠️⚠️ ПУСТО, А НЕ ЗАГЛУШКА. Сюда клался сырой `title`, а в нём
            # у большинства «Тема будет уточнена позже» — и она печаталась на
            # афише, хотя темы нет (у Элины Бутенко она пуста везде).
            # Заполняем ниже, только если тема действительно есть.
            "topic": "",
            "when": when,
            "day": r["day"],

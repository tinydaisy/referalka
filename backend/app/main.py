...
app.include_router(client_brand_logos.router)                                 # /api/v1/clients/me/brand-logos — библиотека логотипов бренда (миграция 449)
app.include_router(collaborator_posters.router)                                # /api/v1/collaborators/{id}/posters — библиотека афиш (миграция 121)
app.include_router(collaborator_photos.router)                                 # /api/v1/collaborators/{id}/photos — библиотека фото (миграция 472)
...
app.include_router(contact_assignments.router, prefix="/api/v1")  # /api/v1/contact-assignments — раздача людей менеджерам лидов (миграция 484)
...
# Частые вопросы техспецов (миграция 460): ОБЩАЯ база готовых ответов —
# завёл один, видят все. Два роутера, потому что раздел открыт и админке,
# и кабинету внедренца; логика одна, отличается только вход.
app.include_router(tech_faq.router,       prefix="/api/v1")
app.include_router(tech_faq.tech_router,  prefix="/api/v1")
...
app.include_router(promo_codes.router,              prefix="/api/v1")  # /api/v1/promo-codes — промокоды клиента (миграция 397)
...
app.include_router(share_image.router)                                          # /api/v1/share-image/event/{slug} — афиша для карточки ссылки в мессенджере

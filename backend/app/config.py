...
class Settings(BaseSettings):
    ...
    # ─── Zoom (клиент подключает свой зум, конференция создаётся кнопкой) ───
    # ⚠️⚠️ ЭТО КЛЮЧИ ПРИЛОЖЕНИЯ ПЛЮСОНа в Zoom Marketplace, ОДНО на всех
    # клиентов — как `vk_oauth_*` и `ig_app_*` выше. Аккаунт Zoom у каждого
    # клиента СВОЙ: он проходит OAuth в кабинете, и его токены ложатся в
    # таблицу `client_zoom_accounts` (миграция 444). Конференции создаются в
    # ЕГО аккаунте — у Zoom лимит одновременных конференций на лицензию, и
    # эфиры разных клиентов в одном аккаунте мешали бы друг другу.
    #
    # ⚠️ Тип приложения — **General App** (OAuth), НЕ Server-to-Server: последний
    # умеет работать только со своим аккаунтом и для клиентов не годится.
    # Ключи берутся на странице приложения, вкладка Basic Information.
    # Нужные scopes: meeting:write, meeting:read, user:read.
    #
    # ⚠️ Пусто → раздел подключения в кабинете не показывается вовсе (не падает
    # при старте и не обещает человеку того, чего нет).
    #
    # ⚠️ Custom Live Streaming (вещание на наш RTMP) требует у аккаунта КЛИЕНТА
    # тарифа Pro и выше — на базовом конференция создастся, а трансляция в
    # вебинарную комнату не включится. Тариф показываем при подключении.
    zoom_client_id: str = ""

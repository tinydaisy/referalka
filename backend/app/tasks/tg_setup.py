...
async def _tick():
    ...
    try:
        ...
        # 1б. ⚠️⚠️ БРОШЕННЫЕ `running` — ВОЗВРАЩАЕМ В ОЧЕРЕДЬ (16.09.2026).
        #
        # Если Celery перезапустить в момент прогона (деплой, OOM, рестарт),
        # задача умирает на середине, а заказ НАВСЕГДА остаётся в `running`:
        # ниже ищется только `queued`, и такой заказ не подбирает никто. Тик
        # продолжает бегать каждую минуту и проходит мимо.
        #
        # Поймано на живом заказе 8 (клиент 191): в 02:34:39 воркер получил
        # SIGTERM сразу после создания бота — и услуга простояла 9 часов, пока
        # человек ждал. В логе Celery: `WorkerLostError: signal 15 (SIGTERM)`.
        #
        # ⚠️ 15 минут — с запасом: живой прогон укладывается в 2–3 минуты, а
        # переписка с BotFather идёт с паузами в десятки секунд. Меньший порог
        # мог бы подхватить заказ, который ещё честно выполняется, и запустить
        # второй прогон параллельно.
        #
        # ⚠️ Слот сервисного аккаунта освобождаем (`setup_account_id=NULL`):
        # иначе он числится занятым брошенной задачей и очередь сужается.
        stuck = await db.fetch(
            """UPDATE service_orders
                  SET setup_state = 'queued', setup_account_id = NULL,
                      updated_at = NOW()
                WHERE setup_state = 'running'
                  AND updated_at < NOW() - INTERVAL '15 minutes'
             RETURNING id, client_id, bot_username"""
        )
        for row in stuck:
            logger.warning(
                "tg_setup: заказ %s висел в running — возвращён в очередь "
                "(клиент %s, бот @%s)",
                row["id"], row["client_id"], row["bot_username"] or "—",
            )
            # ⚠️ В лог заказа пишем ЧЕЛОВЕЧЕСКИ: клиент видит этот лог и должен
            # понимать, что его не бросили, а продолжают с места остановки.
            try:
                await _log_step(db, row["id"], "queue",
                                "Настройка прервалась на середине — "
                                "продолжаем с того места, где остановились")
            except Exception as e:  # noqa: BLE001
                logger.warning("tg_setup: не записал возврат в очередь: %s", e)
...
async def _send_reminder(db, order) -> None:
    ...
    # ⚠️⚠️ СМОТРИМ НА ФАКТЫ, А НЕ НА ПОСЛЕДНЮЮ ОШИБКУ (18.09.2026).
    #
    # Сначала текст выбирался по `setup_error` — и это врало. В группу мы
    # добавляем человека САМИ, сразу после её создания, и у кого приватность
    # открыта, тот оказывается в группе, даже не заходя в бота. У него в
    # `setup_error` лежит «не получилось сделать вас админом», хотя админство
    # — это НАШ следующий шаг, а от него сейчас нужно совсем другое: открыть
    # бота. Проверено на живых заказах: четверо из пяти были в группе и ни
    # один не заходил в бота — все четверо получили бы инструкцию про
    # приватность вместо «зайдите в бота».
    #
    # Порядок важен: пока человек не написал боту, Telegram не отдаст ему
    # бота вовсе — значит это первое, о чём просим.
    if not order["client_started_bot_at"]:
        what_to_do = (
            f"Откройте бота @{order['bot_username']} и нажмите «Запустить» — "
            "без этого Telegram не даст передать вам права владельца."
        )
    elif not order["client_joined_at"]:
        what_to_do = (
            "Осталось вступить в группу уведомлений по ссылке из кабинета — "
            "после этого мы сделаем вас её админом."
        )
    else:
        # В боте был, в группе состоит — значит упёрлись в приватность:
        # назначить админом нам запрещает настройка на его аккаунте.
        what_to_do = (

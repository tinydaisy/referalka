...
async def _handle_add_channel(message: Message, bot: Bot, *, chan_id: Optional[str], url: str, title_hint: Optional[str]) -> None:
    ...
    if not chan_id and url:
        m = url.replace("https://t.me/", "").replace("http://t.me/", "").replace("https://t.me/", "").replace("http://t.me/", "").lstrip("@/").split("/")[0].split("?")[0]
        if m.startswith("+"):
            await message.answer(
    ...
    async with pool.acquire() as db:
        ...
        chan_url = url or (f"https://{TG_DOMAIN}/{title}" if title else "")
        await db.execute(
...
async def on_channel_forward(message: Message, bot: Bot):
    ...
    if not ch or ch.type != "channel":
        ...
        return
    url = f"https://{TG_DOMAIN}/{ch.username}" if ch.username else ""

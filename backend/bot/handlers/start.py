...
async def handle_user_message(message: Message):
    ...
    try:
        ...
        # 6. Ответ пользователю — направляем в Экосистему карточки события,
        # там вся информация об организаторе и его контакты.
        # Логика выбора текста:
        #  - системный @pluson_bot: «откройте приложение → выберите событие → Экосистема»
        #  - VIP-бот клиента:       то же, у бота слева кнопка-меню «Открыть приложение»
        reply = (
            "Спасибо за сообщение, мы передали его организатору 💛\n\n"
            "Чтобы связаться с организатором события напрямую — "
            "откройте приложение, выберите своё событие и зайдите во вкладку "
            "«Экосистема». Там вся информация об организаторе и контакты для связи."
        )

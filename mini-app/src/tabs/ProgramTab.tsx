...
export default function ProgramTab({ event, tgUser, refreshKey, onVipClick, onOpenSpeaker }: Props) {
  ...
  // Какая кнопка красная (миграция 117): 'vip' (дефолт) | 'chat' | 'none'.
  ...
  // Гейт чата — единый хук, общий с WelcomePage.
  // Логика: viability check + user check на каждый клик. См. ChatGate.tsx.
  ...
  // - Конференция: время самой ранней сессии сегодняшнего дня. open_time дня НЕ
  //   используем — в вебе не настраивается и часто неверно.
  ...
  return (
    <div className="fade-in">
      ...
      {/* Чат события — отдельная плашка во всю ширину.
          Цвет управляется events.accent_button (миграция 117): если
          выбран chat — рисуем красным градиентом как у VipButton, иначе
          тёмно-синим (старое поведение). Иконка и шеврон в красном
          варианте — белые, в синем — персиковые. */}
      {hasChat && (
        ...
        <>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 16 }}>
            {days.map(d => {
              ...
              // Формат дня: «ДД.ММ.ГГГГ - Название» (название = кастомное или «День N»).
              const dayName = d.title?.trim() || `День ${d.day_number}`
              const dayLabel = d.day_date
                ? `${fmtDateNumeric(d.day_date)} - ${dayName}`
                : dayName
              ...
              // дня в вебе не настраивается и часто неверно.
              const dayStart = (() => {
      ...
      {/* Модалка проверки подписки для входа в чат — общий компонент, см. ChatGate.tsx */}
      {chatModal}
    </div>
  )
}

...
export function applyTheme(theme: MiniAppTheme): void {
  ...
  // ⚠️ Цвета ПОВЕРХ фирменного фона считает БЭКЕНД (`on_bg_*`): он сверяет
  // яркость и подставляет читаемый вариант, если цвет клиента сливается с
  // фоном. Здесь только раскладываем по переменным — иначе правило контраста
  // существовало бы в двух местах и разъехалось.
  set('--on-dark', theme.on_bg_text || (isDarkColor(theme.bg_color) ? '#ffffff' : '#25455D'))
  // Иконки и стрелки на тёмных плашках (шапка, стрим, кнопки соцсетей).
  set('--on-dark-icon', theme.on_bg_icon || theme.icon)

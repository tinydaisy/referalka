...
/** Пауза перед показом окна: человек успевает увидеть приложение. */
...
export default function VkPermissionsIntro({
  ...
  // ⚠️⚠️ ПАУЗА ПЕРЕД ПОКАЗОМ. Сначала человек видит приложение — календарь
  // или событие, — и только потом всплывает окно. Это и есть «сначала функции,
  // потом запрос прав» из п.1.1.2 правил Mini Apps: раньше окно закрывало
  // экран сразу, за ним была серая пустота, и модерация назвала это «до
  // просмотра функций» (01.09.2026).
  //
  // ⚠️ Паузу не убирать и не уменьшать: за 1-2 секунды приложение не успевает
  // отрисоваться на медленной сети, и окно снова накроет пустой экран.
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* ⚠️ Текст предупреждает о том, что БУДЕТ ДАЛЬШЕ, и объясняет зачем —
            этого и требует п.1.1.2 правил Mini Apps. Формулировка «разрешите
            …» неверна: разрешение человек даёт не здесь, а в системном окне
            ВКонтакте следом. */}
        <p style={{
          fontSize: 15, lineHeight: 1.45, color: fg, fontWeight: 600,
          margin: '0 0 14px', textAlign: 'center',
        }}>
          Сейчас ВКонтакте спросит разрешение на отправку сообщений — оно нужно,
          чтобы вы могли регистрироваться на события и получать напоминания
          о начале и подарки
        </p>

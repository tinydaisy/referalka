...
async function sendVkEventStart(
  ...
  if (askPerms && !(window as any).__vkPermsAsked) {
    ...
    // ⚠️⚠️ ПОДПИСКА — ВНУТРИ КОЛБЭКА, КАК В РАБОЧЕЙ ВЕРСИИ (коммит b9faac81).
    //
    // Именно колбэк `requestWriteAccess` и есть правильный момент: ВКонтакте
    // вызывает его, когда первое окно ЗАКРЫТО, и тут же можно показать второе.
    //
    // ⚠️ Пауза по таймеру вместо колбэка НЕ РАБОТАЕТ — проверено 05.09.2026:
    // и 500 мс, и 12 секунд промахиваются мимо момента закрытия, и VK молча
    // отбрасывает второе окно (он показывает строго одно за раз). Именно так
    // подписка и пропала, когда я заменил колбэк на таймер.
    //
    // ⚠️ Подписка ВНУТРИ блока `askPerms` — то есть только когда человек
    // пришёл ПО ССЫЛКЕ события. На голом входе её не просим: ровно за это ВК
    // снял приложение с публикации (п.1.1.2, 01.09.2026), это путь модератора.
    await new Promise<void>((resolve) => {
      try {

...
async function sendVkEventStart(
  ...
  //
  // ⚠️⚠️ ПОДПИСКУ ПРОСИМ ВСЕГДА — различать входы на стороне Mini App НЕЧЕМ.
  //
  // Проверено на проде 05.09.2026: при заходе по vk.com/app{id}#ref_pg{slug}
  // ВКонтакте присылает приложению ПУСТЫМИ hash, bridge_ref, url_ref и href.
  // То есть `parsed.eventSlug` пуст даже когда человек пришёл по ссылке
  // события, и условие «только по ссылке» не срабатывает НИКОГДА — подписка
  // не запрашивается вовсе.
  //
  // Раньше казалось, что она работает: наше окно-объяснение обрывало запуск и
  // по кнопке «Продолжить» перезапускало его — подписка приходила на втором
  // проходе. Убрали окно — исчезла и подписка. То есть она держалась не на
  // ссылке, а на побочном эффекте перезапуска.
  //
  // ⚠️ ЭТО ОСОЗНАННЫЙ РИСК (решение владельца): автозапрос подписки на входе
  // и есть причина снятия с публикации 01.09.2026 (п.1.1.2). Разделить входы
  // можно только со стороны СЕРВЕРА — он видит vk_ref в параметрах запуска,
  // даже когда фронт хэш не получил. Это отдельная задача.

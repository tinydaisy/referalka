  ...
  useEffect(() => {
    (async () => {
      ...
      if (adapter.name === 'vk') {
        ...
        //
        // ⚠️ Различаем ПО startParam: если он есть — человек пришёл по ссылке
        // (события, лид-магнита, приглашения), и окно-объяснение там не нужно.
        //
        // ⚠️ Прежнее условие смотрело на АДРЕС СТРАНИЦЫ (`parsePathSlug`) — при
        // заходе по ссылке он ещё корневой, поэтому окно показывалось и там
        // (жалоба владельца 05.09.2026). Диагностика на проде подтвердила, что
        // startParam приходит нормально (`ref_pgivision9`), — по нему и решаем.
        // ⚠️ ТОЛЬКО startParam. Адрес страницы (`parsePathSlug`) сюда добавлять
        ...
        // ⚠️⚠️ startParam СЮДА НЕ ДОБАВЛЯТЬ. Диагностика на проде 06.09.2026:
        // при ГОЛОМ входе ВКонтакте присылает `startParam = "other"` — то есть
        // он НЕ ПУСТОЙ. Любая проверка «startParam непустой → пришёл по
        // ссылке» считает голый вход ссылкой и гасит окно-объяснение. Ровно
        // так оно и пропало.
        //
        // Признак «внутри события» — только адрес страницы: при заходе по
        // ссылке событие открывается, и путь становится `/vk/event/{slug}`.
        // ⚠️⚠️ РАЗБИРАЕМ ССЫЛКУ, а не смотрим «строка непустая».
        //
        // При ГОЛОМ входе ВКонтакте присылает `startParam = "other"` (проверено
        // диагностикой 06.09.2026) — непустой, но событием не является. Поэтому
        // проверка «startParam непустой → пришёл по ссылке» гасила окно и на
        // голом входе.
        //
        // Смотрим на СОДЕРЖИМОЕ: у ссылки события есть часть `pg{slug}`, у
        // лид-магнита — префикс `m_`/`p_`, у заглушки — `evl_`. У `other`
        // ничего этого нет.
        //
        // ⚠️ Адрес страницы (`parsePathSlug`) добавлен вторым признаком — для
        // перехода к событию уже внутри приложения. Один только адрес не
        // годится: при заходе по ссылке он ещё корневой, и окно всплывало
        // поверх события (жалоба владельца 06.09.2026).
        const spNow0 = (adapter.startParam || '').trim()
        const linkHasEvent = !!parseStartParam(spNow0).eventSlug
          || spNow0.startsWith('m_') || spNow0.startsWith('p_')
          || spNow0.startsWith('evl_') || spNow0.startsWith('fnl_')
          || spNow0.startsWith('spkinv_') || spNow0.startsWith('prt')
        const byLink = linkHasEvent || !!parsePathSlug()
...
function FunnelStatusScreen({ status, groupId, kind, eventTitle, posterUrl, groupScreen }: {
  ...
  function close() {
    // ⚠️⚠️ СНАЧАЛА ПЕРЕХОД, ПОТОМ закрытие (22.09.2026). Было наоборот:
    // `getPlatform().close()` успевал закрыть Mini App, и присваивание
    // `location.href` выполнялось в уже закрываемом окне — человек оставался
    // на месте, кнопка «НАПИСАТЬ В СООБЩЕСТВО» выглядела сломанной (владелец:
    // «воронка отрабатывает и всё норм — но меня не перебрасывает»).
    // Закрываем только если переходить некуда.
    if (chatUrl) {
  ...
  const TITLE: Record<string, string> = {
    // ⚠️ Не «Подарки будут в чате» (22.09.2026, решение владельца): по этой
    // воронке приходят не только подарки — материалы, инструкции, доступы.
    // Обещать «подарки» там, где придёт инструкция, — обманывать ожидание.
    leadmagnet: 'Информация и материалы будут в чате',
    speaker:    'Код доступа уже в чате',
    partner:    'Инструкция уже в чате',
    event:      eventTitle ? `«${eventTitle}»` : 'Информация будет в чате',
  }
  const BODY: Record<string, string> = {
    leadmagnet: 'Напишите слово «ПОЛУЧИТЬ» в диалоге с сообществом (или нажмите «Начать») — и мы отправим вам всё необходимое.',

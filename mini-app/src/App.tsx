...
async function sendVkEventStart(
  ...
  // ⚠️ Отметка «права уже просили в этом сеансе» — общая с веткой открытия
  // события (openEvent). Без неё человек, пришедший по ссылке, получал бы два
  // одинаковых окна подряд: одно отсюда, другое оттуда.
  // ⚠️ ДИАГНОСТИКА (временная, 05.09.2026): подписка не запрашивается, и по
  // логам не видно, доходит ли выполнение сюда вообще. Пишем состояние
  // развилки: просим ли права, не занята ли отметка, есть ли номер сообщества.
  try {
    fetch(`${import.meta.env.VITE_API_URL}/api/v1/vk/diag-launch`, {
      ...
      body: JSON.stringify({
        ...
        launch_params: {
          ...lp,
          d_askPerms: String(askPerms),
          d_alreadyAsked: String(!!(window as any).__vkPermsAsked),
          d_groupId: String(groupId),
          d_eventSlug: String(parsed.eventSlug || ''),
          d_path: window.location.pathname,
        },
      }),
    }).catch(() => {})
  } catch (_) { /* ignore */ }

  // ⚠️⚠️ БЕЗ УСЛОВИЙ И ОТМЕТОК — ПРЯМОЙ ВЫЗОВ (05.09.2026).
  //
  // Здесь стояли `askPerms` и отметка `__vkPermsAsked` (её ставила и вторая
  // точка — ветка открытия события). Любая из них гасила вызов: подписка не
  // запрашивалась ни разу за день, и по логам не было видно даже попытки.
  //
  // ⚠️ Подписка — ВНУТРИ колбэка `requestWriteAccess`, как в рабочей версии
  // (коммит b9faac81). ВКонтакте вызывает колбэк, когда первое окно закрыто, и
  // тут же показывается второе. Пауза по таймеру вместо колбэка промахивается
  // мимо этого момента, и VK молча отбрасывает второе окно.
  //
  // ⚠️ Эта функция вызывается ТОЛЬКО при заходе по ссылке события — на голом
  // входе права просит окно-объяснение, и подписки там нет (это путь
  // модератора, за неё сняли с публикации, п.1.1.2).
  // ⚠️ Отметка ВРЕМЕНИ (не «навсегда»): вторая точка — ветка открытия события —
  // по ней гасит повторное окно в ближайшие 20 секунд. Вечная отметка тут
  // стояла раньше и гасила запрос насовсем.
  // ⚠️ `askPerms` — единственное оставшееся условие: на ГОЛОМ входе права
  // просит окно-объяснение, и подписки там нет (путь модератора, п.1.1.2).
  // Отметка «уже просили» здесь НЕ проверяется — она стояла вечной и гасила
...
export default function App() {
  ...
  async function openEvent(slug: string) {
    ...
    //
    // `sendVkEventStart` отрабатывает ОДИН РАЗ при запуске приложения, когда
    // события ещё нет: путь тогда корневой, и условие «внутри события» не
    // выполняется. Поэтому по ссылке подписка не приходила вовсе (жалоба
    // владельца 05.09.2026).
    //
    // ⚠️ На голом входе в календарь этот код не выполняется — он висит на
    // ОТКРЫТИИ события. Именно так подписка и не попадает на путь модератора,
    // за который сняли с публикации (п.1.1.2).
    if (getPlatformName() === 'vk') {
      ;(async () => {
        const a = getPlatform()
        if (!a.launchParams?.vk_user_id) return
        // ⚠️ Защита от ДВОЙНОГО окна: права могут запрашиваться и отсюда, и из
        // sendVkEventStart (при заходе по ссылке события). Гасим повтор, если
        // запрос уже уходил меньше 20 секунд назад.
        //
        // ⚠️ Раньше здесь стояла вечная отметка `__vkPermsAsked` — и она
        // ГАСИЛА запрос в sendVkEventStart, если эта ветка успевала первой.
        // Подписка тогда не запрашивалась вообще ни разу (05.09.2026).
        ...
        if (!gid) return
        // ⚠️ Подписка ВНУТРИ колбэка — как в рабочей версии (b9faac81). ВК

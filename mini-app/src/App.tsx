...
async function sendVkEventStart(
  ...
  adapter.requestWriteAccess({ vkGroupId: groupId }, async () => {
    // ⚠️⚠️ ПОДПИСКА НА СООБЩЕСТВО — ТОЛЬКО ПРИ ЗАХОДЕ ПО ССЫЛКЕ СОБЫТИЯ.
    //
    // Эта функция срабатывает на ЛЮБОМ открытии Mini App во ВКонтакте — и по
    // ссылке события (`ref_pg{slug}`), и при голом входе в календарь. Различаем
    // по `parsed.eventSlug`: он заполнен только у ссылки события.
    //
    // ⚠️ Голый вход БЕЗ подписки — это и есть причина снятия с публикации
    // (п.1.1.2: «запрашивает подписку до просмотра функций», 01.09.2026).
    // Условие `if (parsed.eventSlug)` убирать нельзя: без него joinGroup
    // вернётся на календарь, то есть ровно туда, откуда его убрали.
    //
    // ⚠️ Возврат подписки на ссылки — решение владельца (04.09.2026), принятое
    // с предупреждением о риске: модератор может открыть такую ссылку.

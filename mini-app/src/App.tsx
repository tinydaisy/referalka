...
async function sendVkEventStart(
  ...
  //
  // ⚠️⚠️ ТОЛЬКО ПРИ ЗАХОДЕ ПО ССЫЛКЕ СОБЫТИЯ (`parsed.eventSlug` заполнен).
  // На ГОЛОМ входе в приложение подписку не просим — ровно за это ВКонтакте
  // снял приложение с публикации: «запрашивает подписку на сообщество до
  // просмотра его функций» (п.1.1.2, 01.09.2026). Голый вход — тот путь,
  // которым идёт модератор.
  //
  // ⚠️ Условие не убирать «чтобы собрать больше подписчиков»: это возврат к
  // причине снятия.
  if (parsed.eventSlug && adapter.joinGroup && groupId) {
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      ...
      // ⚠️⚠️ ЭКРАН-ОБЪЯСНЕНИЕ ТОЛЬКО НА ГОЛОМ ВХОДЕ (решение владельца
      // 05.09.2026). Человек, пришедший ПО ССЫЛКЕ события или лид-магнита,
      // и так видит, куда попал: у события — лендинг, у лид-магнита — экран
      // «подарки будут в чате». Наш экран поверх этого лишний, права там
      // запрашиваются сразу.
      //
      // ⚠️ На голом входе экран НЕ УБИРАТЬ: п.1.1.2 правил Mini Apps требует
      // объяснить назначение права до системного окна, и за отсутствие такого
      // экрана приложение уже отклоняли (18.06.2026). Голый вход — тот путь,
      // которым идёт модератор.
      const spIntro = adapter.startParam || ''
      const byLink = spIntro.startsWith('ref_pg') || spIntro.startsWith('pg')
        || spIntro.startsWith('evl_') || spIntro.startsWith('m_')
        || spIntro.startsWith('p_') || spIntro.startsWith('fnl_')
        || spIntro.startsWith('spkinv_') || spIntro.startsWith('prt')
      if (adapter.name === 'vk' && !byLink) {

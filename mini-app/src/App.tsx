...
async function sendVkEventStart(
  ...
  if (askPerms && !(window as any).__vkPermsAsked) {
    ...
    // ⚠️⚠️ НЕ ЖДЁМ ОТВЕТА ОТ ПЕРВОГО ОКНА — ЖДЁМ ВРЕМЯ.
    //
    // VK Bridge на ответ полагаться нельзя: колбэк `requestWriteAccess`
    // приходит не всегда (проверено 05.09.2026 — владелец нажимала кнопки, а
    // подписка следом не показывалась ни разу). Пока мы ждали ответ, второе
    // окно не показывалось вовсе.
    //
    // Поэтому просто даём человеку время на первое окно и показываем второе.
    // Ответ первого нам и не нужен: разрешил он или нет, подписку предлагаем
    // всё равно.
    try { adapter.requestWriteAccess({ vkGroupId: groupId }, () => {}) } catch { /* skip */ }
    // ⚠️ 12 секунд: человеку хватает прочитать и нажать, а VK успевает закрыть
    // первое окно. Меньше — второе окно прилетит поверх открытого первого, и
    // VK молча его отбросит (он показывает строго одно окно за раз).
    await new Promise<void>((r) => setTimeout(r, 12000))

    // Помимо разрешения на ЛС — предлагаем подписаться на само сообщество.
    // Это разные действия во ВК: AllowMessages ≠ JoinGroup. group_join на бэке
    // зафиксирует подписавшегося в базе.
    //
    // ⚠️⚠️ ВНУТРИ блока `askPerms` — то есть только когда человек пришёл ПО
    // ССЫЛКЕ события. На голом входе в календарь подписку не просим: ровно за
    // это ВКонтакте снял приложение с публикации («запрашивает подписку до
    // просмотра функций», п.1.1.2, 01.09.2026). Голый вход — путь модератора.
    //
    // ⚠️ Не выносить наружу блока: снаружи условие выполнится и на голом
    // входе, и подписка снова попадёт на путь модератора.
  ...
  async function openEvent(slug: string) {
    ...
    if (getPlatformName() === 'vk') {
      ;(async () => {
        ...
        // ⚠️ ОДИН РАЗ ЗА СЕАНС. Права могут запрашиваться и отсюда, и из
        // sendVkEventStart (когда ссылка события всё-таки дошла) — без этой
        // отметки человек получил бы два одинаковых окна подряд.
        if ((window as any).__vkPermsAsked) return
        ;(window as any).__vkPermsAsked = true
        ...
        // ⚠️ Ответа НЕ ЖДЁМ — колбэк VK Bridge приходит не всегда, и подписка
        // следом не показывалась вовсе. Просто даём время на первое окно.
        try { a.requestWriteAccess({ vkGroupId: gid }, () => {}) } catch { /* skip */ }
        // ⚠️ 12 секунд: человеку хватает прочитать и нажать, а VK успевает
        // закрыть первое окно. Меньше — второе прилетит поверх и будет молча
        // отброшено (VK показывает строго одно окно за раз).
        await new Promise<void>((r) => setTimeout(r, 12000))
        // 2) Подписка на сообщество.
        if (a.joinGroup) {

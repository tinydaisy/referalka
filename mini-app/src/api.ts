...
// Разрешил ли человек писать ему в этом ВК-сообществе.
//
// ⚠️ Бэкенд спрашивает У САМОГО ВКОНТАКТЕ (messages.isMessagesFromGroupAllowed)
// и только при неудаче смотрит нашу базу. У VK Bridge такого метода нет —
// со стороны Mini App узнать можно, лишь показав системное окно.
//
// ⚠️ `groupId` при голом входе бывает пустым (ВК его не передаёт) — тогда
// бэкенд отвечает по базе. Это допустимо: базу с 05.09.2026 пишет только
// событие `message_allow`, то есть факт реального разрешения.
...
export const getEventShareLinks = (slug: string, refCode?: string) => {
  ...
  // ⚠️ КОЛЛАБ: бот, в котором человек СЕЙЧАС, намеренно НЕ передаём. Реф-ссылка
  // привязана к ИСХОДНОМУ организатору участника (в чьей базе он лежит) — в любом
  // боте коллабы он видит СВОИ ссылки. Бэк резолвит организатора по `pid`.

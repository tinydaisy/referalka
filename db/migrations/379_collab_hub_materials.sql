-- 379. Наполнение продукта «Коллабораторная ПЛЮСОН (Материалы)» — записи вебинаров.
--
...
BEGIN
    ...
    IF v_mat IS NULL THEN
        ...
        -- тексте сверху: до просмотра предложение оставить отзыв о материале
        -- бессмысленно.
        (v_mat, 'text', NULL,
         '<h3>Подарок за отзыв — дорожная карта эфира</h3>'
         '<p>Посмотрели материал? Напишите отзыв в чат iViSiON — и мы пришлём вам '
    ...
    -- постоянно и ищут глазами в списке, а не внутри материала.
    v_mat := NULL;
    SELECT m.id INTO v_mat
      FROM materials m JOIN product_materials pm ON pm.material_id = m.id
     WHERE pm.product_id = v_product

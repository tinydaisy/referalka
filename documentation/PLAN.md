...
- [ ] `/dashboard/events/[id]/raffle` (новая вкладка «Розыгрыш»). **Модель — `memory/project_raffle_model.md`** (зафиксировано 2026-05-01):
  ...
  - [ ] У конференций добавить тумблер «Включить розыгрыш» в шапке вкладки «Розыгрыш» — переключает `event_raffle_settings.is_enabled`. Если выключен — подвкладки настроек скрыты, вкладка в Mini App тоже не показывается

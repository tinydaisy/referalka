...
- [ ] `/dashboard/events/[id]/raffle` (новая вкладка «Розыгрыш»). **Модель — `memory/project_raffle_model.md`** (зафиксировано 2026-05-01):
  ...
  - [ ] Подвкладка «Кодовые слова»: CRUD `event_raffle_keywords` — только поле `keyword`. Каждое слово даёт 1 билет участнику. Слово может ввести любое число участников. Сравнение ввода регистронезависимое (`lower(trim(input)) === keyword_lower`), точное по буквам. Повторный ввод тем же участником — «уже использовали»
  - [ ] Билеты участника: число билетов = число введённых слов (включая Free для подписавшихся). Стартовый билет с `code_word='Free'` выдаётся за выполненную подписку
  - [ ] Подписка собирается **только в двух точках**: при заходе в чат события и при заходе в Розыгрыш. При регистрации на событие — НЕ запрашивается. Если участник уже фактически подписан на все требуемые каналы (TG API `getChatMember` или `platform_user_channels`) — чек-лист не показывать, сразу выдавать Free-билет
  - [ ] Один билет = один выигрыш. Уже выигравший билет исключается из последующих рандомов. Но участник с N билетами может выиграть до N призов
  - [ ] Поле `event_participants.live_at TIMESTAMPTZ NULL` — новая колонка, отдельная миграция. Окно «в эфире» = **120 минут** (`live_at > now() - interval '120 min'`)
  - [ ] Live-ссылка: web `https://pluson.ru/l/{slug}?app=tg&live=1` → redirect_web_app → Telegram `t.me/{bot}/pluson?startapp=ref_pg{slug}_live` → Mini App вызывает `POST /api/v1/events/{slug}/live` (auth через initData). Эндпоинт делает upsert `event_participants` с `live_at=now()`; если записи не было — создаёт с `is_registered=false` (интересующийся) и сразу «в эфире»
  ...
  - [ ] **Призов как отдельной сущности НЕТ** — берутся из `conf_speaker_events.gift_raffle_title/url`. Таблица `event_raffle_prizes` (миграция 042) — мертва, не использовать
  - [ ] **Розыгрыш доступен ТОЛЬКО в конференциях** (`module_slug='conference'`). У мероприятий розыгрыша нет — нет спикеров, к которым привязаны призы (зафиксировано 2026-05-01 на MVP)
  - [ ] У конференций добавить тумблер «Включить розыгрыш» в шапке вкладки «Розыгрыш» — переключает `event_raffle_settings.is_enabled`. **Тумблер управляет ТОЛЬКО видимостью у участника в Mini App.** Подвкладки (Настройки / Кодовые слова / Участники / Билеты / Генератор) у организатора всегда открыты для редактирования — он сначала настраивает, потом включает. То же правило для тумблера Реф-программы (`event_referral_settings.is_enabled`, миграция 053) — см. `memory/feedback_dashboard_toggle_only_miniapp.md`
  - [ ] Текст ЛС победителю: «🎉 Поздравляем! Вы выиграли *{gift_raffle_title}* от *{speaker_name}*. {gift_raffle_url ? «Забрать подарок: {url}» : «Чтобы забрать — напишите: @{speaker_tg_username}»}»

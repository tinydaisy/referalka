...
### [x] ⚠️ Фикс внедренца считается за ВСЕХ клиентов — своя база и база ПЛЮСОН не разделены (найдено 2026-09-19, починено 2026-09-22)

Причина была в том, что у внедренца было ДВА реф-кода, а работал только
клиентский: колонка `clients.referred_by_tech_id` не заполнялась никогда.
Починено не заполнением колонки, а её удалением — внедренец стал **ролью
клиента** и приглашает своей клиентской ссылкой (миграции 486–488).
Подробности: [TECH-CABINET.md](architecture/TECH-CABINET.md).

### [x] Внедренцу: реквизиты для выплат + доступ в Авторассыльщик (2026-09-26, миграция 517)

Реквизиты (ФИО, СБП, банк, ИНН, «самозанятый»), выдача аккаунта mailer.pluson.ru
кнопкой из админки, ник владельца для «Запросить доступ». Подробности —
[TECH-CABINET.md](architecture/TECH-CABINET.md).

- [ ] Блокировка аккаунта мейлера — ждём эндпойнт от мейлера
- [ ] Оферта и подписание документов перед выводом денег

### [x] Автораздача участников события менеджерам лидов (сделано 2026-09-22)

Кнопка «Менеджеру лидов» на вкладке «Участники». Правила: привёл внедренец →
ему же; у кого уже есть свой внедренец → не трогаем; остальных поровну.
Плюс в CRM события: кто привёл, метки, даты, фильтр по внедренцам, два блока
ПЛЮСОНа для внедренца. Подробности —
[TARIFFS-ACCESS.md](architecture/TARIFFS-ACCESS.md), раздел «Менеджер лидов».

⚠️ Не путать со следующим пунктом: там распределяются ОБРАЩЕНИЯ в бот, здесь —
УЧАСТНИКИ события между менеджерами внутри кабинета клиента.
...
⚠️ Блокировавшая правка «кто привёл» сделана 22.09.2026 — очередь теперь
опирается на `clients.referred_by_client_id`, можно делать.

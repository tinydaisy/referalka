...
- [x] За 5 минут до начала выступления → «Через 5 минут стартует [Имя спикера] с темой [тема]! 🔥»
- [x] В конце выступления (за 10 минут до следующего или по окончании) → «[Имя спикера] дарит подарок: [название] → [ссылка]»
- [ ] Утром дня конференции → напоминание о программе дня
- [x] Все шаблоны редактируемые в веб-кабинете, с дефолтными значениями (автосоздаются при первом открытии страницы «Шаблоны»)

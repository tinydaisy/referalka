...
#### ⚠️⚠️ ДВА поля `linked_client_id` — это ДВА СЦЕНАРИЯ, а не дубль (23.09.2026)

Поле с одним именем лежит в двух таблицах, и оба **живые**. Удалять ни одно
нельзя: это разные пути «связать себя с кабинетом ПЛЮСОНа», для разных людей.

| | `contacts.linked_client_id` | `collaborators.linked_client_id` |
|---|---|---|
| Кто связывает | **любой участник** (спикером быть не обязан) | **только спикер** события |
| Откуда вызывается | команда **`/pluson_connect`** в боте организатора — TG [start.py:2457](backend/bot/handlers/start.py#L2457), ВК [vk_main.py:1871](backend/bot/vk_main.py#L1871), MAX [max_webhook.py:555](backend/app/api/max_webhook.py#L555) | форма **в кабинете спикера** [speaker/[event_slug]/page.tsx:854](web/src/app/speaker/[event_slug]/page.tsx#L854) → [speaker_cabinet.py:1927](backend/app/api/speaker_cabinet.py#L1927) |
| Что человек делает | пишет команду → жмёт кнопку → вводит email/пароль на `pluson.ru/link-pluson` | открывает свой кабинет спикера → вводит email/пароль (или регистрируется там же) |
| Зачем | все, кого он приведёт на событие, закрепятся **за ним** как за рефоводом | плюс выбор **своего** лид-магнита как «подарка после эфира», турнирный критерий по `funnel_runs`, МедиаЛифт |
| Заполнено на проде (23.09.2026) | 8 из 22958 | 43 из 348 |

**Оба читает резолвер рефовода** — [plusson_referral.py:181](backend/app/services/plusson_referral.py#L181):
`COALESCE(c.linked_client_id, col.linked_client_id)`. Достаточно ЛЮБОГО
непустого: контакт в приоритете, карточка — fallback. Поэтому спикер, связавший
себя через кабинет, рефоводом работает и без записи на контакте.

⚠️ **Почему на контактах почти пусто — не потому, что поле мёртвое.** Чтобы
им воспользоваться, надо ЗНАТЬ про команду `/pluson_connect`: её нет ни в меню
бота, ни в подсказках, человек должен набрать её руками. Механизм рабочий,
до него просто не добираются. В [plusson_match.py](backend/app/services/plusson_match.py)
есть более ранняя запись «на проде так не сделал НИКТО, 0 из 5330» — она про
то же самое и объясняет, почему CRM-блок «кто дошёл до платформы» считает
не по этому полю, а сопоставлением по почте и никам.

⚠️ **Не путать с `clients.referred_by_client_id`.** `linked_client_id` — это
«контакт/карточка = вот этот кабинет». `referred_by_client_id` — «этого клиента
привёл вот тот клиент». Первое отвечает на вопрос «кто ты в ПЛЮСОНе»,
второе — «кто тебя привёл».
...
**Оплата — ещё три записи** (23.09.2026, журнала там не было вовсе):

| Шаг | `platform` | Где |
|---|---|---|
| Заход на лендинг оплаты/регистрации | `{площадка}-page`, иначе `web-page` | `GET /event/{slug}/register` ([event_page_html.py](backend/app/api/event_page_html.py)) |
| Оформление заказа **тарифа** | `{площадка}-order`, иначе `web-order` | `POST /create` ([event_orders.py](backend/app/api/event_orders.py)) |
| Оформление заказа **продукта** | `web-order` | `POST /create` ([product_orders.py](backend/app/api/product_orders.py)) |

⚠️ Заход на лендинг пишется **на открытие страницы**, до всякого заполнения
формы — иначе в журнал попадали бы только купившие, а «зашёл и ушёл» (самое
важное для воронки) терялось бы.

⚠️ Площадка берётся из адреса (`?tg_id=`/`?vk_id=`/`?max_id=`), поэтому запись
одинакова для всех трёх — и из ботов, и из Mini App. У заказа **продукта**
площадки в форме нет вовсе (только `telegram_username` — это ник, не id), там
метка честно `web-order`.

⚠️ В заказе тарифа пишем **сырой `?pid=`** (`raw_param`) и рядом `resolved_ref`
(`parsed_pid`): разойдутся — сразу видно, что код не нашёлся, а не гадать потом.

⚠️ Query-параметр `?pid=` на этом пути **не резался** подчёркиванием (см. раздел
выше) — код приходит отдельным полем. Поломка касалась только слитных payload.

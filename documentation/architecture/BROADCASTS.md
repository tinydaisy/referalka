...
**Чат спикеров — отдельное поле события** (миграция 431): `events.tg_speakers_chat_ref`
/ `vk_speakers_chat_ref` / `max_speakers_chat_ref` → `client_broadcast_chats.id`,
ровно как чаты участников (миграция 174). Три площадки — потому что чат команды
может жить в любой одной из них; заполняется столько, сколько нужно, сообщение
уходит в каждый заполненный. `primary` у них НЕТ: кнопки «Чат» для этого чата не
существует, выбирать главный незачем. В кабинете — раздел **«Чаты и каналы

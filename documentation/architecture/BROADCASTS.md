...
- **Токен.** `wall.post` от имени сообщества умеет только токен человека-админа.
  Берётся одной точкой [vk_admin_token.py](backend/app/services/vk_admin_token.py).
  ⚠️⚠️ Токен VK ID живёт **час**; до 26.09.2026 его не обновлял никто — все пять
  подключённых были просрочены, нативное видео тоже не грузилось. Теперь
  обновление refresh-токеном под `FOR UPDATE` (refresh одноразовый). Нужен
  `device_id` из колбэка — раньше не сохранялся, старые токены надо
  переподключить (плашка в кабинете: `vk_admin_token_needs_reconnect`).
  Права: `video wall photos offline`.
- **Содержимое.** Текст без HTML; кнопки — ссылками в тексте (у поста кнопок
  нет); фото — `photos.getWallUploadServer/saveWallPhoto`; видео — через
  `upload_video_via_user_token` в раздел видео ЭТОГО сообщества, не загрузилось —
  ссылкой в тексте.
...
- **Условия у клиента:** стена сообщества включена; подключивший токен —
  админ/редактор этого сообщества.
- **Репост на личную страницу админа** — галочка в блоке токена
  (`platform_meta.vk_admin_repost`, `PUT /channels/vk/admin-repost`). После
  поста — `wall.repost` тем же токеном («… поделилась записью»). В журнале
  отдельной строкой `chat_ref = repost:{id группы}`: пост мог выйти, а репост нет.
- ⚠️⚠️ **Права выдаёт VK ID только те, что включены в приложении** (id
  54606410, кабинет VK ID для бизнеса → «Доступы»). Не включены — VK молча
  отдаёт токен с `vkid.personal_info offline` (так было 26.09.2026 после
  переподключения). Проверка: `platform_meta.vk_admin_scope` должен содержать
  `wall`, иначе в кабинете плашка «Переподключить».

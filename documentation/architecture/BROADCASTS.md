...
## ⚠️⚠️ Чаты слушаются выбранных площадок (22.09.2026)

«Каналы для отправки» фильтровали только личные сообщения по базе, а чаты — события, спикеров, общие, личные каналы — уходили во ВСЕ площадки, где чат задан. Клиент выбирал Telegram, а пост уходил ещё и в MAX-чат. Ни один из трёх путей отправки в чаты `target_channel_ids` не читал вовсе.

Чат не привязан к каналу-боту (у события это ссылка на `client_broadcast_chats`, у спикеров — прямой `chat_id`), поэтому сверять по `channel_id` нечего. Резолвим ПЛОЩАДКИ выбранных каналов:

```sql
SELECT DISTINCT platform_slug FROM channels WHERE id = ANY($1::int[])
```

и передаём `allowed_platforms` во все три пути ([broadcast.py](backend/app/tasks/broadcast.py)): TG-блок чатов, `_send_broadcast_to_event_chats`, `_send_broadcast_to_client_chats`.

⚠️ **NULL = слать во все** (каналы не ограничивали) — обратная совместимость старых рассылок.

⚠️ **Не смогли определить площадки → шлём как раньше**, а не молча никуда: рассылка важнее фильтра.

⚠️ `BroadcastChannelPicker` получил `hideEmail` и `label`: у письма чата не бывает, и галочка Email в списке, управляющем чатами, только сбивает с толку. Фильтр применяется ДО `onChange` — скрытая площадка не попадёт в `target_channel_ids` даже при автовыборе «все каналы».

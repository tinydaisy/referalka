...
⚠️ **Демо-страница [/dashboard/help/step-zero-preview](web/src/app/dashboard/help/step-zero-preview/page.tsx) НЕ подключена к меню** — открывается только по прямой ссылке. Работает по-настоящему (письмо уходит, бот отмечает подписку). Встроим шаг в автонастройку — страница удаляется, разметка переезжает в `AutoSetupTab`.

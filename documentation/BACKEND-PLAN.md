...
- **`chat_platforms JSONB`** у `broadcast_schedules` и `broadcast_templates` (миграция 520) — площадки по видам чатов `{private, common, event, speakers}` → список площадок; нет ключа = все. «Каналы для отправки» на чаты не влияют. Что подключено у каждого вида — `GET /api/v1/events/{id}/broadcasts/chat-platforms` и `GET /api/v1/broadcasts/chat-platforms`. Подробности — [BROADCASTS.md](architecture/BROADCASTS.md).
- **`clients.link_mode_{telegram|vk|max}`** (миграция 200, `miniapp|bot`, NOT NULL DEFAULT `'bot'`) + `start_mode`, `start_event_id`, `start_greeting_text`, `start_btn_events_label`, `start_btn_owner_label` — куда ведут публичные ссылки/кнопки бота + приветствие /start.
  ⚠️ «Общего режима» (`default_link_mode`, миграция 169) **больше нет** — удалён миграцией 478 (20.09.2026): переключателя для него в кабинете не существовало, значение проставлялось само, а код читал его вместо площадочной настройки — и выбор «Telegram: Mini App» не действовал.

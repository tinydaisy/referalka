...
# ── 7. Отметка «что реально разложено» ────────────────────────────────────
# ⚠️ По git HEAD судить о версии прода НЕЛЬЗЯ: коммит могли сделать прямо на
# сервере — тогда HEAD уже новый, а сборка в web/.next ещё старая, и deploygit
# решал «всё стоит, обновлять нечего» и молча пропускал накат. Отметку пишем
# только здесь — после того, как сборка переключена и прошла проверку.
echo "${NEW_SHA:0:8}" > "$PROJECT_DIR/.deployed_sha" 2>/dev/null || true

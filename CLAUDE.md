...
⚠️ **Одним длинным листом, а не набором A4** — лендинг это сплошное полотно, разрывы по высоте A4 пришлись бы на середину карточек. Высота меряется у готовой страницы. Потолок Chrome ~200 дюймов: длиннее — режется на несколько листов (лучше, чем ошибка).
...
⚠️ **Сборка занимает 8–12 секунд**, поэтому кнопка показывает «Собираем PDF…» и блокируется: иначе клиент жмёт её повторно и запускает вторую печать поверх первой.

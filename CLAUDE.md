...
⚠️ **Платный тариф ведёт на СУЩЕСТВУЮЩУЮ `/e/{slug}/order/{id}`** — там оплата, «Это вы?» и приём данных. Второго механизма оплаты не заводим. Бесплатный открывает ту же форму, что была за кнопкой раньше.

⚠️ **Контакт и реф-код тащим с собой** (`?c=`, `?pid=`, `?utm_source=`): без них человек на странице заказа станет «новым», а приведший его — никем.

⚠️ **Адрес заказа — на домене клиента** (`client_public_base` из ответа `/events/{slug}/landing`), литералов `pluson.ru` во фронте нет.

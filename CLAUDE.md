...
⚠️ **Услуга считается оказанной, когда бот СОЗДАН.** Всё дальнейшее зависит от действий клиента. Не забрал за **3 дня** — бот удаляется, слот освобождается, но **оплата НЕ сгорает**: клиент запускает настройку заново без повторного платежа (`setup_state='expired'`, кнопка «Запустить заново — бесплатно»).

⚠️ **Условие про 3 дня показывается ДО оплаты**, а не после — иначе споры.
...
2. **получатель уже написал этому боту** — иначе его нельзя выбрать получателем;
3. подтверждение — **облачным паролем** нашего аккаунта (лежит в `tg_setup_accounts.twofa_password`).
...
⚠️ **Забрать бота — 3 дня.** Не забрал → бот удаляется, слот освобождается,
**оплата не сгорает**. Напоминаний максимум 4.
...
|---|---|---|
| **LeadPay** | есть, в кабинете (раздел «Промокоды») | их справка **прямым текстом**: «Промокоды действуют на карточки товара, созданные в LeadPay. Работаете через интеграцию? Реализуйте функционал на своей стороне». Мы идём через интеграцию — поля ввода в нашем потоке нет вовсе |
...
| **LeadPay** | он по сети (`getLink`) | **из карточки товара** — цену не передать |

⚠️ **У LeadPay есть ВТОРОЙ метод — `rest/v3/salebot/link`**, который принимает `product_name` и `product_price` прямо в запросе, без карточки товара. При промокоде уходим туда. **Проверить живым вызовом до выкатки** — метод описан в их интеграции с Salebot, надо убедиться, что он доступен нашими ключами и что вебхук приходит в том же формате.

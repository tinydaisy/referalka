...
Раздел **«Лендинг»** в карточке события (все типы: мероприятие, конференция, турнир, конкурс) — продающая страница собирается из блоков, без Tilda/GetCourse. Гейт — фича **`event_landing`** (входит в **Профи, Экстра и триал** — сверять с БД, а не с этой строкой), у КОЛЛАБ-события скрыт.

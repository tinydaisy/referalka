...
Клиент ставит `CNAME свой-домен → pluson.ru` (или A-запись на наш IP — некоторые провайдеры не дают CNAME на корне), жмёт «Проверить DNS», затем «Выпустить сертификат». В карточке видно срок действия и остаток дней.

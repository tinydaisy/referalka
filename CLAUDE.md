...
**«Рассылки со мной» — новая вкладка в кабинете спикера** ([/speaker/<slug>](web/src/app/speaker/%5Bevent_slug%5D/page.tsx), компонент `MyBroadcastsTab`). Показывает рассылки этого события, привязанные к спикеру (`broadcast_schedules.session_id = event_collaborators.id` спикера): speaker_intro / 5min_before / gift / expert_day. Только статусы `pending` («В очереди») и `done` («Отправлено») — **черновики (`draft`) не показываем**. У каждой — кнопка-глазик → модалка-превью (текст+фото/видео+кнопка), собранная тем же `build_message_content`. Эндпоинт `GET /api/v1/public/speaker-cabinet/me/my-broadcasts` ([speaker_cabinet.py](backend/app/api/speaker_cabinet.py)).

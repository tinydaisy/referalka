...
|---|---|---|
| Новый интерес | рефоводу (чей код в ссылке) | имя, ник ссылкой, площадка, время |
| Покупка | рефоводу + **почта** | кто, что, сумма, **процент**, начислено, баланс |
| Новый клиент платформы | **основателю ПЛЮСОНа** | имя, почта, телефон, TG, кто привёл |

⚠️ **ВСЁ уходит В ЛИЧКУ от @pluson_bot.** Каналы уведомлений клиента (`notifications_telegram_chat_id`) и его VIP-бот тут **НЕ участвуют**: VIP-бот — про события клиента (у Марго первым идёт `@ivision_conf_bot`, уведомление о ПЛЮСОНе приходило от бота конференций), а канал клиент заводит руками и сам добавляет туда бота — `@pluson_bot` в канал клиента 1 не добавлен, отправка падала `chat not found`. Реф-программа обязана работать у КАЖДОГО клиента сразу, без настройки.

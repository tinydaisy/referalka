...
| `vip`   | Экстра | 2990 ₽ | топ |
| `business_beta` | Бизнес Beta | 4900 ₽ | **скрыт** до карточек LeadPay (миграция 365) |
...
⚠️⚠️ **Тариф создан СКРЫТЫМ (`is_active = FALSE`)** — карточек LeadPay ещё нет, а без них оплата невозможна: цена лежит в карточке, а не в запросе. Показать раньше значило бы дать нажать «Оплатить 4900 ₽» и получить отказ — **кнопка на странице подписки активна всегда и `payable` не проверяет** ([subscription/page.tsx](web/src/app/dashboard/subscription/page.tsx), в отличие от блока модулей, где кнопка скрывается). Включение — одной строкой, порядок в шапке миграции; суммы для карточек 4900 / 25 872 / 47 040 ₽.

...
| Команда | Что делает |
|---|---|
| **`deploygit`** | ОСНОВНАЯ. Находит последнюю **успешную** сборку `main` на GitHub, качает готовый архив, отдаёт в `release.sh`. На проде ничего не собирает. Сборка идёт — скажет подождать (`deploygit --wait` дождётся сам); упала — не выкатит вовсе |
...
|---|---|
| `web/` | `plusson-web` |
| `backend/app/api\|services\|main.py` | `plusson-api` |

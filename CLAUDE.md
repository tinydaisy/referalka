...
### ⚠️⚠️ Сертификат pluson.ru — GlobalSign от reg.ru, НЕ Let's Encrypt (2026-08-19, ПРОД)
...
⚠️ **Зачем вообще ZeroSSL:** его корень — старый Sectigo `USERTrust`, известный и старым устройствам, в отличие от нового корня Let's Encrypt `ISRG Root YR` (13 мая 2026), из-за которого у части посетителей сайт не открывается вовсе.
...
⚠️ **Ещё не на проде** — миграция написана, не накачена. В работе: сервис готов, API и формы заказа — нет.
...
**Файлы:** [382_promo_codes.sql](db/migrations/382_promo_codes.sql), [promo_codes.py](backend/app/services/promo_codes.py). Расчёт скидки тарифа (не путать) — [tariff_discount.py](backend/app/services/tariff_discount.py).

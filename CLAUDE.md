...
⚠️ **`skip_contact_form` работает и в вебе, и в Mini App — одинаково (2026-07-28, ПРОД).** Раньше флаг читал только Mini App ([EventPage.tsx](mini-app/src/pages/EventPage.tsx) `handleWantParticipate`), а веб-путь его не знал вовсе: кнопка «ЗАРЕГИСТРИРОВАТЬСЯ» из бота ведёт на `pluson.ru/event/{slug}/register?c={contact_id}`, и та страница всегда рисовала форму. Теперь в `event_register_page` ([event_page_html.py](backend/app/api/event_page_html.py)) есть ветка: `skip_contact_form=TRUE` + контакт известен из `?c=` и принадлежит клиенту события → UPSERT `event_participants(is_registered=TRUE)` + `finalize_participant_registration` → 302 на `/event/{slug}?c={contact_id}` (кабинет), форма не показывается. Поле добавлено в SELECT `_resolve_event`. Точка одна — покрывает TG ([start.py](backend/bot/handlers/start.py)), MAX ([max_webhook.py](backend/app/api/max_webhook.py)) и VK разом, все ведут на этот URL. Без `?c=` (человек открыл ссылку не из бота, контакт неизвестен) форма остаётся — регистрировать некого. Согласия при таком входе не фиксируются — как и в Mini App.

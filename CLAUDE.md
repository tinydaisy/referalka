...
Теперь: пуш → GitHub собирает у себя → на сервер едет архив **~25 МБ** → [release.sh](deploy/release.sh) раскладывает его РЯДОМ со старой сборкой и переключает. Секунды вместо минут; не прошла проверка `/health` — автооткат на предыдущую версию.
...
⚠️ **Деплой запускается ТОЛЬКО вручную** (`workflow_dispatch` в Actions), в том числе с телефона. Автодеплой по пушу намеренно выключен — подрядчик пушит код, выкатывает владелец. Есть отложенный вариант «ночью в 04:00 МСК»: сборка ждёт в `/var/www/plusson/pending`, переключает `plusson-release.timer` (GitHub ждать 12 часов не может). Отмена — `rm -f /var/www/plusson/pending/*`.

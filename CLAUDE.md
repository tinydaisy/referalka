...
**Автогенерация** (`POST /dashboards/{id}/autofill`): «мне лень тыкать эти квадратики». Создаёт разом квадратики по всем подходящим разрезам. **⚠️ Только `select`/`multiselect`/`bool`/`number`/`scale`** (`AUTO_KINDS`) — у свободного текста почти все ответы уникальны, полоски по одному человеку это мусор. Повторный вызов дубли не плодит.

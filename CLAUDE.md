...
#### Уведомления рефоводу: «новый интерес» и «покупка» (2026-08-14, ПРОД)

Реф-программа самого ПЛЮСОНа **молчала**: человек заходил по `?start=ref<код>`, код за ним закреплялся — а рефовод не узнавал об этом никогда. Уведомления в ветке `ref<код>` не было ни на одной площадке. Единая точка — [plusson_referral_notify.py](backend/app/services/plusson_referral_notify.py).

⚠️ **Адресат — РЕФОВОД (чей код в ссылке), а не владелец бота.** Это разные клиенты: человек может прийти по ссылке Марго в чужой VIP-бот. Партнёрская программа принадлежит рефоводу.

| Событие | Куда | Что внутри |
|---|---|---|
| Новый интерес | TG + MAX + VK (`notify_organizer_all_channels`) | имя, ник ссылкой, площадка, время |
| Покупка | те же + **почта** (`kind='payments'`) | кто, что, сумма, **процент**, начислено, баланс |

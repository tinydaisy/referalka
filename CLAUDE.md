...
|---|---|---|
| 5 узбекских (`99870038815x/2xx`) | нет | ✅ работает |
| `13654095820` (Канада), `16693142567` (США) | есть | ❌ отказ |

⚠️ **Личные аккаунты владельца в услуге не участвуют.** Массовое создание ботов — как раз то, за что банят, а на личном висят свои боты и каналы: блокировка отобрала бы управление ими навсегда. Аккаунты под услугу — расходники.
...
⚠️ **Пароль двухфакторки виден в админке НАМЕРЕННО.** Аккаунты — расходники: сгорел один, завели другой. Владельцу нужно уметь зайти руками, а для этого нужны телефон и пароль. Держать их в `.env` значило бы перевыпускать конфиг и рестартить сервис на каждую замену аккаунта. Ключи **приложения** Telegram (`TG_SETUP_API_ID` / `TG_SETUP_API_HASH`) — наоборот в конфиге: они одни на все аккаунты.
...
Партнёр видит вкладками «1-й уровень / 2-й уровень», кто под ним стоит (вкладка «Моя сеть» в [PartnerCabinet.tsx](web/src/app/my/PartnerCabinet.tsx), `GET /public/partner/me/network`). **Клиент видит ТОТ ЖЕ разрез**, разворачивая карточку партнёра в разделе «Партнёры» (`GET /partner-program/partners/{id}/network`): без этого разговор «почему мне начислено столько» вести не с чем — цифра есть, а из чего сложилась, не видно.
...
⚠️ **У клиента сеть грузится ПО КЛИКУ на карточке, а не списком при открытии вкладки.** Партнёров бывают сотни, и рекурсивный запрос на каждого положил бы страницу.

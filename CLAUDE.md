...
⚠️ **Найдено попутно, НЕ починено:** у клиентского домена `www.peregovorka.online` TLS не отвечает — в автогенерируемом конфиге (`write_site_config` в [domain_cert.py](backend/app/services/domain_cert.py)) в `server_name` только голый домен, без `www`, и сертификат выписан тоже без него. DNS у `www` при этом ведёт на нас, и люди по нему заходят (видно по `Referer` в логах). Чинить нужно в генераторе + перевыпуском сертификата на два имени.

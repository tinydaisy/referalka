 ...
 * У владельца — сводка по всем внедренцам (`mode="admin"`, есть выпадашка
 * выбора), у внедренца — выборка по нему самому (`mode="tech"`, выпадашки нет:
 * сервер сам подставляет его номер из токена). Две копии экрана разошлись бы
 * в правилах, и воронка показывала бы разное по одним и тем же людям.
 */
export default function CrmScreen({ mode }: { mode: 'admin' | 'tech' }) {
  ...
  useEffect(() => {
    // ⚠️ Список внедренцев нужен ТОЛЬКО владельцу: у внедренца выбора нет,
    // и запрос к админской ручке ему всё равно вернул бы 403.
  ...
  useEffect(() => {
    ...
    setLoading(true)
    const t = setTimeout(() => {
      // ⚠️ Свой адрес на каждый кабинет: у внедренца сервер подставляет его
      // номер из токена, параметром он не передаётся — иначе можно было бы
      // подставить чужой и увидеть чужих клиентов.
      (mode === 'admin'
        ? api.adminTech.crm({ spec_id: specId, status, q, limit: PAGE, offset })
        : api.tech.crm({ status, q, limit: PAGE, offset }))
        .then((r: any) => {
          if (!alive) return
          setItems(r.clients || [])
          setTotal(r.total || 0)
  ...
  function syncUrl(nextSpec: number | null, nextView: 'board' | 'list') {
    ...
    if (nextSpec && mode === 'admin') s.set('spec', String(nextSpec))
    if (nextView === 'list') s.set('view', 'list')
    const qs = s.toString()
    // ⚠️ У каждого кабинета свой адрес: `router.replace` на чужой путь увёл бы
    // внедренца в админку, куда его всё равно не пустят.
    const base = mode === 'admin' ? '/admin/tech/crm' : '/tech/crm'
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-gray-900">
        {mode === 'admin' ? 'CRM клиентов' : 'Моя CRM'}
      </h1>
      <p className="mt-1 text-sm text-gray-500">
        {mode !== 'admin'
          ? 'Ваши клиенты по этапам пути: закреплённые и приведённые вами лично.'
          : current

...
interface LinkRow {
  ...
  platform?: 'telegram' | 'vk' | 'max' | 'web' | 'reg' | 'landing'
  label: string
...
export default function PublicLinks({
  ...
  const buildRows = (pl: PlatformLinks, kind: 'miniapp' | 'bot'): LinkRow[] => {
    ...
    // ⚠️ РЕГИСТРАЦИЯ БЕЗ МЕССЕНДЖЕРА (2026-08-28). Форма существовала давно, но
    // ссылку на неё в кабинете не показывали — на неё вели только боты и
    // рассылки, и клиент про неё не знал. Нужна тем, у кого аудитория не в
    // мессенджерах, и тем, у кого ботов нет вовсе.
    rows.push({
      // ⚠️ СВОЯ площадка 'reg', не 'web' (24.09.2026). Обе строки были помечены
      // 'web', и галочка получалась ОДНА НА ДВЕ страницы: снимаешь у простой
      // страницы — пропадает и форма регистрации. А это разные вещи: витрина
      // события и форма записи, и выключать их нужно порознь.
      key: `${kind}-reg`, platform: 'reg', label: 'Регистрация без мессенджера', badge: 'РЕГ', color: '#6b7c8e',
      url: `${publicBase}/event/${slug}/register`,
      hint: 'Форма прямо на сайте: имя, почта, телефон. После записи предложим перейти в бота. Для реферальной ссылки допишите ?pid=КОД',
    })

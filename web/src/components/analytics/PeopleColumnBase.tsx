...
export interface ColumnPerson {
  ...
  /** Кто привёл человека НА СОБЫТИЕ (реферовод события). */
  referrer_name?: string | null
  /** Кто привёл человека В ПЛЮСОН — по чьей реф-ссылке он зашёл в @pluson_bot.
   *  ⚠️ Обычно это ДРУГОЙ человек, чем реферовод события. */
  plusson_referrer_name?: string | null
    ...
    <Link href={hrefFor ? hrefFor(person)
      ...
      <span className="min-w-0 flex-1">
        <span className="flex items-center gap-1.5">
          ...
          {/* ⚠️ Пилюли «свой» здесь НЕТ намеренно (решение владельца
              22.09.2026). Она имела смысл только для внедренца, а вкладка CRM
              общая для всех клиентов: у кабинета, не связанного с ПЛЮСОНом,
              реферовод-контакт и аккаунт помощника не совпадают никогда, и
              метка не показалась бы вовсе. А «покажи моих» уже решено
              закреплением: менеджер лидов видит ТОЛЬКО своих людей, и пилюля
              «свой» в списке, где все свои, ничего не сообщает. Кто привёл —
              видно строкой «привёл: Имя». */}
        ...
        {/* ⚠️⚠️ ДВА РАЗНЫХ «ПРИВЁЛ», И ОНИ НЕ ВЗАИМОЗАМЕНЯЕМЫ (23.09.2026).
            Раньше стояла одна подпись «привёл: Имя», и по ней было не понять,
            привёл человека НА СОБЫТИЕ или В ПЛЮСОН. Для внедренца это разные
            вещи: от второго зависят его начисления, а люди в этих двух ролях
            обычно разные. Пишем раздельно и называем, куда именно. */}
        {person.referrer_name && (
          <span className="block truncate text-[11px] text-gray-500">
            привёл на событие: {person.referrer_name}
          </span>
        )}
        {/* Персиковым, а не серым: это про ПЛЮСОН — то, ради чего внедренец
            сюда и смотрит, и теряться в сером оно не должно. */}
        {person.plusson_referrer_name && (
          <span className="block truncate text-[11px] font-medium" style={{ color: '#B57A3C' }}>
            привёл в ПЛЮСОН: {person.plusson_referrer_name}
          </span>
        )}

...
const PLATFORMS: { slug: string; label: string; auto?: boolean }[] = [
  ...
  // ⚠️ «База» ВЕРНУЛАСЬ в список (18.09.2026). Её убрали как «считается
  // автоматически», но у старых карточек актив `database` остался в данных —
  // и форма его не показывала. При сохранении он уходил ПУСТЫМ, а бэкенд
  // отвечал «У медийного актива database не указано число подписчиков»:
  // карточка не сохранялась вовсе, и починить это из интерфейса было нельзя.
  { slug: 'database',  label: 'База контактов' },

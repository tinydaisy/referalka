      ...
      */}
      {introPassed && (!order || startAnother || (paid && !order.bot_username && !inProgress
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-5">
          ...
          {/* ⚠️ Условие про срок показываем ДО оплаты, а не после — иначе споры. */}
          <div className="mt-5 rounded-lg bg-gray-50 border border-gray-200 px-4 py-3 text-sm text-gray-600">
            ...
            это два нажатия. {/* ⚠️ Срок дробный (1,5 суток) — «1.5 дней»
            читается коряво, поэтому переводим в часы, когда это не целое
            число дней. Значение приходит с сервера, здесь не хардкодим. */}
            Забрать бота нужно за{' '}
            <b>{Number.isInteger(state.claim_days ?? 0)
              ? `${state.claim_days} дн.`
              : `${Math.round((state.claim_days ?? 0) * 24)} ч.`}</b>{' '}
            — иначе он удаляется, и настройку придётся запустить заново
            (повторно платить не нужно). Перед удалением предупредим.
          </div>
      ...
      {expired && (
        <div className="rounded-xl border border-amber-300 bg-amber-50 p-5 mb-5">
          ...
          <p className="text-sm text-amber-900/80 mb-4">
            {order?.setup_error || 'Бота не забрали вовремя, и он был удалён.'}
          </p>

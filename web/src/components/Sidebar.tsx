...
export default function Sidebar() {
  ...
  // Пункт «Закрытый чат» Коллабораторной (миграции 264 и 266). С 25.09.2026
  // виден НЕ всем: сервер отдаёт ссылки только оплатившим VIP события 89
  // (хардкод в collab_hub.py, _has_collab_chat_access). Пустые ссылки → пункта нет.
  // Ошибку глотаем — ломать сайдбар из-за этого нельзя.
  ...
  const sections = isOrdersAssistant ? ordersSections : isLeadsAssistant ? leadsSections : [
    ...
    ...(hasCollabHub ? [{
      ...
      items: [
        ...
        // Закрытый чат — только оплатившим VIP события 89 (см. hasCollabChat выше).
        ...(hasCollabChat
        ...
        {supportOpen && (
          <div className="ml-4 pl-3 border-l border-white/10 mt-0.5 mb-1 space-y-0.5">
            ...
            {/* ⚠️ Стоит НАД «Инструкциями» намеренно: это «сделайте за меня»,
                ...
                ⚠️⚠️ ПОМОЩНИКАМ НЕ ПОКАЗЫВАЕМ (22.09.2026, решение владельца).
                Помощник работает в ЧУЖОМ кабинете, а автонастройка — это заказ
                услуги от имени клиента: она трогает его бота, его BotFather и
                его деньги. Уровни `leads` / `orders` / `limited` скрыты, у
                `full` пункт остаётся — это доверенный помощник, действующий
                под кабинет клиента, где он помощник. `isAssistant` уже означает
                «помощник, но НЕ full». */}
            {!isAssistant && (
            <Link

              ...
              {['speakers', 'partners', 'values', 'difference', 'gallery', 'numbers', 'audience', 'tariffs'].includes(block.kind) && (
                <>
                  ...
                  {/* ⚠️ Выбор ВЕРСИИ ЛОГОТИПА (миграция 479). В карточке
                      партнёра-компании их два: «для тёмного фона» и «для светлого».
                  ...
                  {block.kind === 'partners' && (
                    <Field label="Логотип у партнёров-компаний">
                      <div className="flex flex-wrap gap-2">
                        {/* ⚠️ Слова «основной логотип» НЕ СУЩЕСТВУЕТ: у
                            компании логотип «для тёмного фона» и «для светлого
                            фона», так они и подписаны в карточке партнёра.
                            Значение `main` в базе остаётся техническим именем
                            колонки `photo_url` — переименовывать его ради
                            подписи незачем. */}
                        {([
                          ['main', 'Для тёмного фона'],
                      ...
                      <p className="mt-1 text-xs text-gray-500">
                        ...
                        «Люди». Фон под логотипами тёмный — берите версию «для
                        тёмного фона», светлый — «для светлого». Нужной версии у
                        партнёра нет — покажется вторая, пустой карточка не
                        останется.
                      </p>
                    </Field>
                  )}
                    ...
                    {block.logo_bg === 'none' ? (
                      ...
                      <>
                        ...
                        <p className="text-xs text-gray-500">
                          ...
                          вкладке «Содержимое» версию «для тёмного фона».
                        </p>
                      </>
                    )}
                  </div>

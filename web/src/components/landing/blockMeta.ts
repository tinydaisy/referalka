...
export const BLOCK_META: Record<BlockKind, BlockMeta> = {
  ...
  survey: {
    ...
    repeatable: true,
    // ⚠️ ЗАГОЛОВКА И ПОДЗАГОЛОВКА ЗДЕСЬ НЕТ (07.09.2026): они задаются один
    // раз в самой форме заявки («Платежи/Заявки» → «Формы заявки») и оттуда
    // приходят на страницу. Два места для одного текста разъезжаются — как и
    // было с выбором анкеты. В блоке остаётся только ОФОРМЛЕНИЕ секции.
    fields: ['survey'],
  },
  gallery: {
    kind: 'gallery',

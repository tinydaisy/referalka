...
export const BLOCK_META: Record<BlockKind, BlockMeta> = {
  hero: {
    kind: 'hero',
    // ⚠️ «Главная страница», а не «Шапка»: рядом есть настройка «Меню и
    // логотип наверху страницы», и два «шапки» в одном списке путались.
  ...
  gifts: {
    ...
    hint: 'Подарки и пороги — из вкладки «Реф-программа» события, подвкладка «Подарки». Правите там — на лендинге обновляется само.',
    live: true,
  ...
  speakers: {
    kind: 'speakers',
    label: 'Спикеры',
    hint: 'Спикеры — из раздела «Люди» события. Поправили карточку — на лендинге обновилось само.',
    live: true,
  ...
  tariffs: {
    kind: 'tariffs',
    label: 'Тарифы',
    hint: 'Тарифы и цены — из вкладки «Тарифы» события, с кнопками оплаты. Тарифов нет — секция не показывается.',
    live: true,
    fields: ['title', 'subtitle', 'button'],
  },
  survey: {
  ...
  venue: {
    ...
    hint: 'Адрес — из поля «Адрес места проведения» на вкладке «Основное» события (появляется по галочке «Офлайн-событие»). Метку на карте ставим по координатам из подсказки адреса. Секция включается и выключается этой же галочкой.',
    live: true,
    fields: ['title', 'body'],
  },
  support: {

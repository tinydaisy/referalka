...
type LegalData = {
  ...
  /** Готовый текст политики с подставленными реквизитами — считает сервер. */
...
/** Готовый текст политики — ПРИХОДИТ С СЕРВЕРА (`policy_template`).
 *
 *  ⚠️⚠️ СВОЕЙ КОПИИ ТЕКСТА ЗДЕСЬ БОЛЬШЕ НЕТ (16.09.2026). Шаблон жил в этом
 *  файле, но автонастройка «под ключ» публикует политику сама, на сервере, —
 *  и две копии одного юридического документа неминуемо разошлись бы: у
 *  клиентов оказались бы РАЗНЫЕ политики в зависимости от того, заполнили они
 *  её руками или это сделала услуга. Источник один:
 *  `backend/app/services/privacy_policy.py`, сюда он приезжает готовым.
 *
 *  ⚠️ Там же живёт обязательный по 152-ФЗ пункт о месте обработки данных
 *  (серверы на территории РФ) — в старом фронтовом тексте его не было вовсе. */
function buildPolicyTemplate(d: Partial<LegalData> | null): string {
  return d?.policy_template || ''
}

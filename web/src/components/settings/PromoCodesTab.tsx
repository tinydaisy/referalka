...
export default function PromoCodesTab({ payProvider }: Props) {
  ...
  // ⚠️⚠️ ЗАМКА ПО LEADPAY БОЛЬШЕ НЕТ (15.09.2026). Он стоял, пока LeadPay
  // работал через старый getLink: там цена лежала в карточке товара, и скидку
  // передать было нечем. Теперь боевые ссылки идут через v2 — название и цена
  // уходят в самом запросе, карточки товаров не нужны вовсе.
  //
  // ⚠️ Список поддерживающих систем живёт на СЕРВЕРЕ (SUPPORTED_PROVIDERS в
  // services/promo_codes.py). Здесь его не дублируем: ровно из-за копии фронт
  // и сервер разъехались — сервер уже принимал промокоды с LeadPay, а вкладка
  // была закрыта.
  ...
  return (
    <div className="space-y-5">
      ...
      {/* ⚠️ Пишем ЧЕСТНО, что проверено живой оплатой, а что нет: обещать
          работу там, где её не подтверждали, — прямой путь к жалобам
          «код не принимается». */}
      {payProvider && (
        <div className="rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 text-sm text-gray-600">
          Промокоды проверены живой оплатой на Т-Банке.
          {payProvider === 'leadpay' && (
            <> С LeadPay работают через его новый способ оплаты — цена со скидкой
            уходит в самом запросе, заводить карточки товаров не нужно. Первую
            оплату стоит сделать на некруглую сумму с промокодом: она сразу
            покажет, что скидка дошла.</>
          )}
          {payProvider === 'prodamus' && (
            <> С Продамусом должны работать, но живой оплатой мы это ещё не
            проверяли.</>
          )}
        </div>
      )}

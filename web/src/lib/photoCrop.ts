...
export function cropStyle(
  ...
  let left = maskW / 2 - W * (fx / 100) + (dx / 100) * maskW
  let top = maskH / 2 - H * (fy / 100) + (dy / 100) * maskH

  // ⚠️⚠️ НЕ ДАЁМ КАРТИНКЕ ОТОЙТИ ОТ КРАЁВ МАСКИ. Если точка лица стоит высоко
  // (а она почти всегда в верхней трети), картинка уезжает вниз и СВЕРХУ
  // остаётся пустота — маска не заполнена, а фото выглядит обрезанным снизу.
  // Прижимаем к краю: лицо оказывается не ровно в центре, но кадр заполнен

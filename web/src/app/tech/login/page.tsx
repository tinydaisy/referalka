...
/**
 * Вход тех-специалиста (внедренца).
 *
 * ⚠️ Ходит в ОБЩИЙ `/auth/login`, а не в свою ручку: там уже разобраны все роли
 * по порядку (клиент → помощник → тех-спец → админ). Отдельный эндпоинт означал
 * бы вторую копию проверки пароля.
 *
 * ⚠️⚠️ ССЫЛКИ «ЗАБЫЛИ ПАРОЛЬ?» ЗДЕСЬ НЕТ НАМЕРЕННО. Восстановление пароля
 * умеет только три роли — клиент, админ, помощник: в `password_reset_tokens`
 * есть `client_id` / `admin_id` / `assistant_id` и ограничение «ровно один из
 * трёх», а `password-reset/request` почту тех-специалиста не ищет вовсе.
 * Ссылка вела бы в тупик: человек ввёл бы почту и получил «такой почты нет».
 * Поэтому пишем правду — пароль выдаёт и меняет владелец платформы.
 */
    ...
    <div className="min-h-screen flex">
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center px-6 py-12 bg-white">
        <div className="w-full max-w-md">
          ...
          {/* ⚠️ Вместо ссылки «Забыли пароль?» — куда идти на самом деле:
              восстановления для этой роли нет (см. шапку файла). */}
          <p className="mt-6 text-center text-sm text-gray-500">
            Забыли пароль? Его выдаёт владелец платформы — напишите ему,
            и он пришлёт новый.
          </p>
        </div>
      </div>

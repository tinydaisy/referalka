...
export default function CollaborationsPage() {
  ...
  async function handleDelete(id: number, name: string) {
    if (!confirm(`Удалить «${name}» из базы коллабораций?`)) return
    try {
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Коллаборации</h1>
          <p className="text-gray-500 mt-1">Спикеры, партнёры, жюри — все, с кем вы работаете</p>
        </div>
        <Link
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-16 text-center">
          ...
          <h2 className="text-xl font-bold text-gray-900 mb-3">
            {query ? 'Ничего не найдено' : 'База коллабораций пуста'}
          </h2>
          <p className="text-gray-500 mb-8 max-w-sm mx-auto">
            {query
              ? 'Попробуйте другой запрос или добавьте новую коллаборацию.'
          ...
          {!query && (
            ...
            >
              <Plus size={16} /> Добавить первую коллаборацию
            </Link>
          )}
        </div>
      ) : (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          <div className="border-t border-gray-100">
            ...
            >
              ...
              <span>Добавить коллаборацию</span>
            </Link>
          </div>
        </div>
      )}

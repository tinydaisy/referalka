      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        return (
          <div key={s.slug}
            ...
            {/* ⚠️ Нулевая цена показывается словом «Бесплатно», а не «0 ₽»:
                ноль читается как незаполненное поле или сбой. */}
            <div className="mt-3 mb-1 flex items-baseline gap-2">
            ...
            {/*
              ...
              а не пользование. Иначе получивший доступ видит «скоро будет».

              ⚠️ Ссылка ведёт СРАЗУ НА ВКЛАДКУ автонастройки (`?tab=autosetup`),
              а не просто в «Каналы»: там три вкладки, и человек попадал на
              «Боты», где ничего про услугу нет, и искал её сам.

              ⚠️ Кнопки оплаты нет намеренно — пока идёт обкатка, услуга не
              продаётся, а подключается по коду доступа. Вернуть продажу =
              снять `coming_soon` в админке и заменить эту ветку на кнопку

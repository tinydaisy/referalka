...
export default function VkSetupInstructionPage() {
  ...
  return (
    <div className="pb-24 max-w-3xl">
      ...
      {/* ⚠️ Главный смысл для клиента — до технических шагов. Во ВКонтакте
          преимущество сильнее, чем в Telegram: по ссылкам события человек
          становится и подписчиком сообщества, не только получателем сообщений. */}
      <div className="rounded-xl border p-5 mb-4" style={{ background: 'rgba(255,207,164,0.15)', borderColor: PEACH }}>
        <div className="text-base font-bold mb-2" style={{ color: BRAND }}>
          Зачем это нужно: подписчик появляется сам, до регистрации
        </div>
        <p className="text-sm text-gray-700 leading-relaxed">
          Человек переходит по ссылке на ваше событие, открывает Mini App — и <b>сразу
          становится подписчиком вашего сообщества</b>, ничего для этого не делая
          специально. При этом он видит удобный интерфейс события: программу, спикеров,
          подарки, чат — а не форму, которую надо заполнить, чтобы хоть что-то посмотреть.
        </p>
        <p className="text-sm text-gray-700 leading-relaxed mt-3">
          <b>Так вы получаете максимальный охват:</b> в вашей базе остаются даже те,
          кто просто интересовался и не зарегистрировался. На обычном лендинге такой
          человек уходит навсегда — здесь вы можете написать ему позже, напомнить о
          событии и позвать снова.
        </p>
        <p className="text-xs text-gray-600 mt-3">
          Сообщество при этом становится каналом для рассылок и приветствий, а Mini App —
          приложением со всеми возможностями: программа, реферальные подарки, экосистема.
        </p>
      </div>
      ...
      <Section step="5" title="Включить возможности бота">
        ...
        <p className="text-sm font-semibold text-gray-800 mb-2">
          В <strong>правом меню управления сообществом</strong> найдите пункт <strong>«Настройки для бота»</strong>{' '}
          (он в списке между «Сообщения» и «Чаты»). Откройте его.
        </p>
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-3 text-sm text-blue-900 mb-3">
          💡 <strong>Не видите пункт «Настройки для бота»?</strong> Тогда откройте раздел{' '}
          <strong>«Сообщения» → подвкладка «Настройки»</strong> (в шапке раздела) и прокрутите до блока{' '}
          <strong>«Возможности ботов»</strong> — это одно и то же, просто у разных сообществ ВК показывает по-разному.
        </div>
        <p className="text-sm text-gray-700 mb-2">На открывшейся странице «Настройки для бота»:</p>
        <ol className="text-sm text-gray-700 space-y-1.5 list-decimal pl-5 mb-4">
          <li>Строка <strong>«Возможности ботов»</strong> должна показывать статус{' '}
            <strong>«Включены»</strong>. Если написано «Выключены» / «Отключены» — нажмите на эту надпись
            и включите. <span className="text-xs text-gray-500">(Это ссылка-статус, а не галочка — просто должно стоять «Включены».)</span></li>
          <li>Поставьте галочку <strong>«Добавить кнопку „Начать"»</strong> ✅ — <strong>это обязательно</strong>.
            <span className="block text-xs text-gray-500 mt-0.5">
              Аналог /start в Telegram. Когда человек впервые открывает диалог с сообществом,
              ВК покажет ему большую синюю кнопку «Начать». Он её жмёт → ПЛЮСОН получает событие и шлёт первое сообщение.
              Без этой галочки воронка лид-магнита не запустится.
            </span>
          </li>
          <li>Галочку <strong>«Разрешать добавлять сообщество в чаты»</strong> — на ваше усмотрение
            (можно оставить включённой, на воронки не влияет).
          </li>
          <li>Нажать <strong>«Сохранить»</strong></li>
        </ol>
      ...
      <Section step="7.5" title="⚠️ ГЛАВНОЕ: включить приложение «для всех» (иначе ссылки не откроются)">
        <div className="bg-red-50 border-2 border-red-300 rounded-xl p-4 text-sm text-red-900 mb-4">
          <p className="font-bold mb-2">Без этого шага у людей будет ошибка:</p>
          <p className="mb-1">• <strong>«Мини-приложение доступно только его администраторам»</strong> — приложение выключено / в тестовом режиме.</p>
          <p>• <strong>«Игра заблокирована администрацией сайта»</strong> — приложение не прошло проверку / не опубликовано.</p>
          <p className="mt-2">У вас как у админа ссылка открывается — а у клиентов нет. Проверять всегда в режиме инкогнито или с чужого аккаунта.</p>
        </div>
        <p className="text-sm text-gray-700 mb-2">
          Свежесозданное Mini App по умолчанию <strong>видно только вам</strong>. Чтобы им могли
          пользоваться подписчики (и работали ссылки на лид-магниты), переведите его в публичный статус:
        </p>
        <ol className="text-sm text-gray-700 space-y-1.5 list-decimal pl-5 mb-3">
          <li>Откройте <a href="https://dev.vk.com" target="_blank" rel="noreferrer" className="text-blue-600 hover:underline font-medium">dev.vk.com</a> → ваше приложение → <strong>«Настройки»</strong></li>
          <li>Найдите раздел <strong>«Состояние приложения»</strong> (или «Доступ» / «Видимость»)</li>
          <li>Поставьте <strong>«Приложение включено и видно всем»</strong> (не «Только администраторам» и не «Тестовый режим»)</li>
          <li>В разделе <strong>«Настройки» → «Основные»</strong> убедитесь, что у приложения задано <strong>название</strong> и <strong>иконка</strong> — без них ВК не публикует приложение</li>
          <li>Сохраните и <strong>проверьте ссылку в инкогнито</strong> (или попросите знакомого открыть) — должна открыться воронка, а не замок</li>
        </ol>
        <div className="bg-red-50 border-2 border-red-300 rounded-xl p-4 text-sm text-red-900 mt-4">
          <p className="font-bold mb-2">🔴 САМОЕ ВАЖНОЕ — пройти модерацию VK (иначе ссылки на лид-магниты не работают у людей)</p>
          <p className="mb-2">
            Пока приложение <strong>не проверено VK</strong>, при первом переходе по ссылке
            человек видит белый экран <strong>«Сервис создан сторонними разработчиками и ещё не
            проверен → Запустить»</strong>. Если он жмёт «Запустить» с этого экрана —{' '}
            <strong>VK теряет ссылку на конкретный лид-магнит</strong>, и вместо подарка открывается
            обычное приложение со списком событий. Это ограничение VK, обойти его нельзя — нужно,
            чтобы приложение стало <strong>проверенным</strong>.
          </p>
          ...
          <ol className="list-decimal pl-5 space-y-1">
            ...
            <li>Нажмите <strong>«Отправить на модерацию»</strong> (или «Опубликовать»)</li>
            <li>VK проверяет обычно <strong>1–3 дня</strong>. После одобрения экран «Запустить» исчезнет.</li>
          </ol>
          <p className="mt-2 text-xs">
            💡 До прохождения модерации ссылки на лид-магниты в VK будут работать нестабильно
            (у части людей откроется список событий вместо подарка). Это нормально до одобрения —
            дождитесь проверки VK.
          </p>
        </div>
      </Section>

...
// ⚠️⚠️ Рассылки В ЧАТ СПИКЕРОВ — ОДИН список на весь кабинет (24.09.2026).
// У них получатель ОДИН и задан в настройках события, поэтому ни аудитории, ни
// каналов, ни галочек «чаты события / общие чаты / личные каналы» им показывать
// нельзя: это выбор, которого нет, а снятая галочка ещё и ломала доставку.
// Раньше условие было написано под один `speakers_call` — и у «Программы дня» с
// «Инструкцией по подключению» весь этот блок продолжал висеть. Тот же список
// живёт в очереди (queue/page.tsx) и на бэке (set_fire_at / update_template).
const SPEAKERS_CHAT_TYPES = ['speakers_call', 'speakers_day', 'speakers_howto']
...
const ALL_VARIABLES: { name: string; desc: string }[] = [
  ...
  { name: '{signup_link}', desc: 'Ссылка «Зарегистрироваться» ПОД ПЛОЩАДКУ ПОЛУЧАТЕЛЯ: тому, кто читает в Telegram, придёт телеграм-ссылка, в ВК — вэкашная, в MAX — максовская. Своей площадки у события нет — придут все имеющиеся, каждая с подписью («Через Телеграм»); нет ни одной — веб-страница регистрации. Работает и на событиях, и на премиях. В КНОПКЕ письма разворачивается в три отдельные кнопки — по одной на площадку' },
...
const INTRO_ROLE_OPTIONS: { value: string; label: string }[] = [
  ...
  // ⚠️ Генеральный партнёр — ОТДЕЛЬНАЯ роль в БД (`general_partner`), а не
  // разновидность партнёра (23.09.2026). Галочки для него тут не было вовсе, и
  // отметить «Партнёры» не помогало: фильтр сверяет роль точным совпадением —
  // генеральные партнёры молча не попадали в «Знакомство со спикером».
  { value: 'general_partner', label: 'Генеральные партнёры' },
...
const CUSTOM_PLACEHOLDERS = [
  ...
  // ⚠️ Ссылка регистрации ПОД ПЛОЩАДКУ получателя — в отличие от
  // {landing_url}, который один на всех. В подсказках её не было вовсе,
  // хотя рассылка её подставляет (23.09.2026).
  '{signup_link}',
...
export default function TemplatesPage() {
  ...
  //
  // ⚠️ Это про РАСПИСАНИЕ, и `speakers_call` сюда НЕ входит намеренно: она
  // уходит перед каждым выступлением, время у неё считается от слота спикера,
  // а не «во сколько отправить». Список типов чата спикеров — отдельный
  // (`SPEAKERS_CHAT_TYPES`), он про аудиторию и каналы; не путать.
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-xl p-6 max-h-[90vh] overflow-y-auto scroll-visible">
            ...
            <div className="space-y-3">
              ...
              {/* ⚠️ У рассылки в чат спикеров выбирать нечего: получатель ОДИН
                  ...
                  ⚠️⚠️ ВСЕ ТРИ типа чата спикеров, а не один (24.09.2026): условие
                  было написано под `speakers_call`, и у «Программы дня» с
                  «Инструкцией по подключению» продолжали висеть аудитория,
                  каналы и галочки «чаты события / общие чаты / личные каналы» —
                  настройки, которые для них ничего не значат. Список типов —
                  один и тот же, что в очереди и на бэке (set_fire_at). */}
              {SPEAKERS_CHAT_TYPES.includes((form as any).type) ? (

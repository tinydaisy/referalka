 ...
 * опознаёт человека (`platform=contact`) и отдаёт его кабинет. Без параметра
 * зритель неизвестен, поэтому и показывается форма.
 *
 * ⚠️ Это НЕ отдельный «режим предпросмотра», а настоящие страницы: вторая
 * ссылка открывает кабинет от лица КОНКРЕТНОГО человека — самого спикера. Своей
 * регистрации у него может не быть, тогда он тоже увидит лендинг: это правда, а
 * не поломка, и об этом сказано в подписи.
 */
    ...
    <div className="bg-blue-50 border border-blue-200 rounded-2xl p-4 space-y-3">
      ...
      {memberUrl ? (
        ...
        <div className="text-xs text-gray-500">
          Второй ссылки нет: у этого человека не заведён контакт в базе, а кабинет открывается именно по нему.
        </div>
      )}
      <div className="text-[11px] text-gray-500 leading-snug">
        Вторая ссылка открывает кабинет от лица этого человека. Если он ещё не зарегистрирован
...
export default function ConferenceSpeakerPage() {
  ...
  async function saveProfile(e: React.FormEvent) {
    ...
    try {
      // Лимиты: не даём сохранить полотно — на лендинге и в карточке спикера
      // место рассчитано на название, а не на абзац.
      if ((profile.title || '').length > POSITIONING_LIMIT) {
        alert(`Позиционирование слишком длинное — сократите на ${(profile.title || '').length - POSITIONING_LIMIT} символов`)
        return
      }
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-4">
          ...
          <div>
            ...
            <input type="text" value={profile.title || ''} onChange={setP('title')}
              className={`w-full px-4 py-2.5 rounded-xl border text-sm focus:outline-none focus:border-brand ${overClass(profile.title || '', POSITIONING_LIMIT) || 'border-gray-200'}`} />
            <CharCount value={profile.title || ''} limit={POSITIONING_LIMIT} />
          </div>
          <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <div className="flex items-center w-full rounded-xl border border-gray-200 focus-within:border-brand overflow-hidden">
                ...
                <input type="text"
                  value={profile.assistant_tg_username || ''}
                  onChange={(e) => {
                    const v = e.target.value.replace(/^@+/, '').trim()
                    setProfile((f: any) => ({ ...f, assistant_tg_username: v }))
                  }}

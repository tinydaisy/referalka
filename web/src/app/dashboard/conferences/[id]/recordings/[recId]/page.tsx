...
export default function RecordingCutPage() {
  ...
  // ⚠️⚠️ АВТОСОХРАНЕНИЕ МЕТОК (24.09.2026). Раньше метки жили ТОЛЬКО в
  // браузере до нажатия «Сохранить»: обновил страницу — работа на десятки
  // минут пропала. Предупреждение beforeunload при F5 срабатывает не всегда,
  // и владелец так потерял всю расстановку.
  //
  // ⚠️ С паузой в 3 секунды после последнего действия, а не на каждый клик:
  // метки ставят пачками, и запрос на каждую перегрузил бы сервер.
  //
  // ⚠️ Ошибку глотаем молча: не вышло сейчас — попробуем через 3 секунды,
  // а кнопка «Сохранить» и пометка «есть несохранённые изменения» остаются
  // как запасной путь. Пугать человека алертом посреди работы нельзя.
  const autoSaveRef = useRef<any>(null)
  useEffect(() => {
    if (!dirty || busy) return
    clearTimeout(autoSaveRef.current)
    autoSaveRef.current = setTimeout(() => { save().catch(() => {}) }, 3000)
  ...
  return (
    <div className="max-w-5xl pb-10">
      ...
      <div className="flex flex-wrap gap-2 items-center mb-3">
        ...
        {/* ⚠️ Подсказка ОБЪЯСНЯЕТ, почему кнопка серая (24.09.2026). Раньше
            title был один на все случаи, и человек видел просто серую кнопку:
            владелец решил, что генератор обложек вообще пропал из раздела. */}
        <button onClick={buildCovers} disabled={!!busy || dirty || !cuts.length}
                title={
                  busy ? 'Идёт другая операция — подождите'
                  : dirty ? 'Сначала сохраните метки: обложки собираются по сохранённым точкам'
                  : !cuts.length ? 'Сначала расставьте метки — обложки собираются по ним'
                  : 'Собрать обложки по шаблону «Спикер» и посмотреть их'
                }
                className="px-4 py-1.5 rounded-lg border text-sm flex items-center gap-1.5 disabled:opacity-40">

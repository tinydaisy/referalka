...
export default function RecordingCutPage() {
  ...
  return (
    <div className="max-w-5xl pb-10">
      ...
      {/* ⚠️ Пояснение прямо над списком, а не только в подсказке внизу: без него
          кнопка «начать с 0:00» выглядит загадкой — непонятно, откуда это время
          и что произойдёт. */}
      {cuts.length > 0 && (
        <div className="mb-2 text-xs text-gray-500">
          Красная полоса на таймлайне сейчас на <b className="tabular-nums text-[#25455D]">{mmss(curLive)}</b>.
          Двигайте её, пока на видео не появится начало нужного выступления, — и нажмите
          «начать с …» в строке этого спикера, чтобы граница встала ровно туда.
        </div>
      )}
        ...
        {!cuts.length ? (
          <div className="py-4 text-sm text-gray-500 leading-relaxed">
            {hasOffset ? (
              <>
                ...
                если метка встала не туда — нажмите «начать с …» в её строке.
              </>
            ) : (
              <>
                <div className="font-medium text-[#25455D] mb-1">Нажмите «Расставить по программе дня»</div>
          ...
          return (
            <div key={c._k ?? c.id ?? i}
              ...
              {/* ⚠️ Кнопка подписана ПОЛНОСТЬЮ, а не значком «↓ сюда»: тот сам
                  по себе ничего не говорил, и было непонятно, куда «сюда».
                  Показываем и действие, и конкретное время. */}
              {c.status !== 'ready' && (
                <button onClick={() => patch(i, { start_sec: Math.round(curLive) })}
                        className="px-2.5 py-1 rounded-md border border-[#25455D] text-[#25455D] text-xs shrink-0
                                   hover:bg-[#25455D] hover:text-white transition-colors"
                        title="Начало этого куска станет тем местом, где сейчас стоит красная полоса на таймлайне">
                  начать с <span className="tabular-nums font-medium">{mmss(curLive)}</span>
                </button>
              )}
              <input
      ...
      <p className="mt-5 text-xs text-gray-500 leading-relaxed">
        ...
        «начать с …» в строке нужного спикера, и граница встанет ровно туда. Метку можно и тянуть по полосе мышью

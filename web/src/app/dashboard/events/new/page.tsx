...
export default function NewEventPage() {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    try {
      const res = await api.events.create({ title: title.trim(), module_slug: 'base' })
  ...
  return (
    <div className="max-w-lg mx-auto">
      <div className="flex items-center gap-3 mb-10">
        <Link href="/dashboard/events" className="p-2 rounded-lg hover:bg-gray-100 text-gray-500 transition-colors">
          <ArrowLeft size={18} />
        </Link>
      ...
      <form onSubmit={handleCreate} className="space-y-5">
        ...
        >
          {loading ? <><Spinner /> Создаём...</> : 'Создать мероприятие'}
        </button>

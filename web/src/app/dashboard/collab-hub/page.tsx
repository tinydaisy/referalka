...
import { Handshake, Search, Star, Send, Inbox, Sparkles, MapPin, Check, X, Users, Award, ExternalLink, Pencil } from 'lucide-react'
  ...
  return (
    <div className="p-4 md:p-8 max-w-6xl mx-auto">
      ...
      <div className="flex gap-1 mb-6 border-b overflow-x-auto">
        {([['card','Моя карточка',Star],['catalog','Каталог',Search],['requests','Запросы',Inbox],['matchmaker','Умный сват',Sparkles]] as [Tab,string,any][]).map(([key,label,Icon]) => (
          <button key={key} onClick={() => setTab(key)}
            className={`flex items-center gap-1.5 px-4 py-2.5 text-sm font-medium whitespace-nowrap border-b-2 transition ${tab===key?'border-current':'border-transparent text-gray-400 hover:text-gray-600'}`}
...
function CollabCard({ item, onRequest }: { item: any; onRequest?: () => void }) {
  return (
...
// ── Моя карточка = профиль организатора (clients) ──
function MyCardTab() {
  ...
  const load = async () => {
    ...
    setCard(r.card)
    setForm({
      is_published_in_hub: r.card.is_published_in_hub ?? true,
      hub_category: r.card.hub_category || '', hub_niche: r.card.hub_niche || '',
      hub_city: r.card.hub_city || '', hub_about: r.card.hub_about || '',
    })
  }
  useEffect(() => { load().catch(()=>{}); api.collabHub.niches().then((r:any)=>setNiches(r.niches||[])).catch(()=>{}) }, [])
  const save = async () => {
    setErr('')
    try { await api.collabHub.publishCard(form); setSaved(true); setTimeout(()=>setSaved(false),2000); load() }
  ...
  return (
    <div className="grid lg:grid-cols-2 gap-8">
      {/* Превью карточки — профиль организатора */}
      <div>
...
function CatalogTab() {
  ...
  const load = async () => {
    setLoading(true)
    try { const clean=Object.fromEntries(Object.entries(f).filter(([,v])=>v)); const r:any=await api.collabHub.catalog(clean as any); setItems(r.items||[]) } catch { setItems([]) }
    setLoading(false)
  }
  ...
  return (
    <div>
      ...
      {loading ? <div className="text-gray-400 py-10 text-center">Загрузка…</div>
        ...
        : <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">{items.map(it=><CollabCard key={it.client_id} item={it} onRequest={()=>setReqTarget(it)} />)}</div>}

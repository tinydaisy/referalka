      ...
      <div className="max-w-6xl mx-auto p-3 md:p-5 grid md:grid-cols-[1fr,340px] gap-4">
        ...
        <div>
          ...
          <div className="mt-3 space-y-2">
            {(room.blocks || []).filter((b: any) => b.kind === 'button' || b.kind === 'form').map((b: any) => (
              <div key={b.id} className="rounded-xl bg-white/10 p-3">
                ...
                {b.kind === 'button' ? (
                  <button onClick={() => clickBlock(b)} className="w-full py-2 rounded-lg text-sm font-semibold" style={{ background: '#FFCFA4', color: '#0a1520' }}>
                    {b.title || 'Подробнее'}
                  </button>
                ) : (

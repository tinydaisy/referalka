...
export default function WebinarRoomPage() {
      ...
      if (Hls.isSupported()) {
        ...
        const mk = () => {
          ...
          const h = new Hls({
            ...
            // ⚠️⚠️ 2 СЕГМЕНТА, А НЕ 3 — РАДИ БЫСТРОГО СТАРТА (26.09.2026).
            // Плеер отступает от живого края на это число сегментов и ДОЛЖЕН их
            // набрать, прежде чем показать картинку. При 3 это 6 секунд ожидания,
            // на мобильной сети больше: зритель видит чёрный экран и уходит
            // («люди так ждать не будут» — правило владельца). 2 сегмента = 4 с.
            // ⚠️ Ниже 2 опускать нельзя: у плеера не остаётся запаса на джиттер,
            // и картинка начинает дёргаться на каждой заминке сети.
            ...
            // ⚠️⚠️ БУФЕР ПО РАЗМЕРУ ПЛЕЙЛИСТА, А НЕ «ПОБОЛЬШЕ» (26.09.2026).
            //
            // Стояло 30/60 секунд, а в плейлисте живого эфира всего 14 (7
            // сегментов по 2 с). Плеер вечно недобирал буфер до желаемого и
            // сообщал `bufferStalledError` — в логе он был У ВСЕХ, кто играл.
            // Снаружи это выглядело как «нервно дёргается» и «зацикленный плей»:
            // видео шло рывками, а сторож то показывал кнопку, то убирал.
            //
            // ⚠️ 14 секунд при плейлисте 22 с (11 сегментов по 2 с). Ставили 10
            // при плейлисте 14 с — плеер задыхался у самой границы и присылал
            // `bufferNudgeOnStall` (видно в логе presence): «долго грузится →
            // плей → мелькает → чёрная тишина» на Android Яндексе. Запас должен
            // быть МЕНЬШЕ плейлиста, но не у его края.
            // ⚠️ Менять вместе с `hlsSegmentCount` в mediamtx.yml: это ОДНА
            // настройка, разнесённая по двум файлам. Увеличишь буфер, не увеличив
            // плейлист — вернётся bufferStalledError.
            maxBufferLength: 14,
            maxMaxBufferLength: 24,
            // ⚠️ Стартуем, как только есть хоть что-то: без этого плеер ждёт
            // полный `maxBufferLength` перед первым кадром — те самые «долго
            // грузится, чёрный экран» на Android.
            maxBufferHole: 0.5,
   ...
   */
  /**
   * ⚠️⚠️ СТОРОЖ «КАРТИНКА ВИСИТ БЕЗ ОШИБКИ» (26.09.2026, правило владельца).
   *
   * Кнопка «Смотреть эфир» показывалась только по `playerStuck`, а он ставится
   * ЛИШЬ при ошибке плеера. Но самый частый случай другой: видео просто стоит,
   * ошибки нет вовсе — и зритель видит чёрный прямоугольник БЕЗ кнопки и без
   * крутилки. Догадаться, что надо ткнуть в экран, неоткуда: «после кучи раз
   * нажатий на экран пошло движение» — так это выглядело у зрителя на Android MI.
   *
   * Поэтому смотрим на ФАКТ: растёт ли `currentTime`. Не растёт 6 секунд при
   * живом эфире → поднимаем кнопку.
   *
   * ⚠️ Порог 6 секунд, а не 2: на мобильной сети короткие остановки — норма, и
   * кнопка, выскакивающая на каждом джиттере, мешала бы смотреть.
   *
   * ⚠️ Опрос, а НЕ событие: события 'waiting'/'stalled' приходят не во всех
   * браузерах, и именно поэтому прошлый сторож (снятый 24.09) строился на
   * опросе. Его убрали из-за петли «нажал → буферизация → снова кнопка»;
   * здесь петли нет, потому что `playerLoading` скрывает кнопку, пока идёт
   * загрузка, а сам сторож смотрит на рост времени, а не на `readyState`.
   */
  useEffect(() => {
    let prev = -1
    let stalls = 0
    const t = setInterval(() => {
      const v = videoRef.current
      // ⚠️ Проверяем наличие <video>, а не флаг `live`: он объявляется НИЖЕ по
      // файлу (после условных return), и зависимость от него не собралась бы.
      // Пока эфира нет, <video> не отрисован — сторож просто ничего не делает.
      if (!v || document.hidden || !v.src && !hlsInstRef.current) return
      const cur = v.currentTime || 0
      // ⚠️⚠️ ВИДЕО ИДЁТ → КНОПКУ СНИМАЕМ СРАЗУ (26.09.2026). Кнопка «Смотреть
      // эфир» висела ПОВЕРХ ИДУЩЕГО видео («твой плей показывается уже на
      // идущем видео» — владелец): сторож ставил её, а снять мог только на
      // следующем тике через 2 секунды, и то лишь при росте времени. Теперь
      // проверяем `paused` первым делом: не на паузе и буфер есть — кнопки быть
      // не должно, независимо от счётчика.
      if (!v.paused && v.readyState >= 2) { stalls = 0; prev = cur; setPlayerStuck(false); return }
      // ⚠️⚠️ ВИДЕО ИДЁТ РЫВКАМИ — КНОПКУ НЕ ПОКАЗЫВАЕМ (26.09.2026).
      // При голодающем буфере (`bufferStalledError`) плеер на мгновение встаёт и
      // снова идёт. Сторож успевал поднять кнопку, плеер — продолжить, и кнопка
      // «нервно дёргалась» поверх живого видео (Android Chrome). Считаем
      // остановкой только то, что длится: пока плеер не на паузе, ждём дальше.
      if (!v.paused) { prev = cur; return }

...
export async function generateMetadata(
  ...
  //
  // Порядок афиш (горизонтальная → квадратная → вертикальная) задан в SQL,
  // который отдаёт `poster_url`; здесь остаётся только «афиша или логотип».
  //
  // ⚠️⚠️ АФИША ОТДАЁТСЯ ЧЕРЕЗ `/api/v1/share-image/...`, А НЕ ПРЯМОЙ ССЫЛКОЙ
  // НА ХРАНИЛИЩЕ (24.09.2026, прод). Мессенджер не берёт тяжёлую картинку: у
  // Telegram предел превью 1280 px и 5 МБ, у WhatsApp около 600 КБ. У реального
  // события афиша была 4320×2430 и 4,2 МБ — в чате показывался логотип при
  // полностью верном `og:image`, и выглядело это как «афиша не подставилась».
  // Эндпоинт ужимает её на лету, поэтому карточка работает с ЛЮБОЙ афишей —
  // и с уже лежащими в хранилище тоже, без пересборки руками.
  const host = headers().get('host') || ''
  const ogImage = event.poster_url
    ? (host ? `https://${host}/api/v1/share-image/event/${params.slug}` : event.poster_url)
    : brandLogo

...
export default function OrderForm({
  ...
  // ⚠️⚠️ ПЛАШКА ПЕРСИКОВАЯ, ТЕКСТ ТЁМНО-СИНИЙ — и прозрачность ЛЁГКАЯ (.85).
  // Посчитано, а не на глаз: страница тёмная (#0F1E2E), и персиковый на ней
  // при сильной прозрачности перестаёт быть персиковым —
  //   .18 → #3a3e43 (серый, контраст с синим 1.1:1 — текста не видно);
  //   .55 → #937f6f (бурый, 2.6:1 — плохо);
  //   .85 → #dbb492 (персиковый, 5.2:1 — читается).
  // От сходства с кнопкой уходим не прозрачностью, а формой: у плашки нет
  // рамки и тени, текст не заглавный и не жирный, скругление как у блоков.
  const hintBg = (() => {
    const hex = page.border_color || page.color_heading || '#FFCFA4'
    const m = /^#?([0-9a-f]{6})$/i.exec(hex)
    if (!m) return 'rgba(255,207,164,.85)'
    const n = parseInt(m[1], 16)
    return `rgba(${(n >> 16) & 255}, ${(n >> 8) & 255}, ${n & 255}, .85)`
  })()
  // Текст — тёмно-синий из брендинга: на персиковом он единственный читаемый.
          ...
          {tariff.order_hint && (
            <div className="mb-4 px-5 py-3.5 text-center text-[.85em] font-semibold leading-snug"
                 style={{
                   ...
                   color: hintFg,
                   // ⚠️ Скругление БЛОКОВ (`radius`, у клиента 1 это 8), а не
                   // кнопок (34): плашка — это блок, и с кнопочным скруглением
                   // она ещё сильнее походила на вторую кнопку.
                   borderRadius: page.radius ?? 8,
                 }}>
              {tariff.order_hint}
            </div>
          )}

...
function PartnerCard({
  ...
  // человека висит в нём маленьким прямоугольником в пустоте (жалоба со
  // скриншотом).
  //
  // ⚠️⚠️ Признак человека — ФАМИЛИЯ, и только она. Должность для этого НЕ
  // годится: на проде она заполнена и у организаций («Федеральное медиа-
  // агенство» у Love Media, «Медиа-платформа для экспертов» у Грани) — по
  // ней логотип уехал бы в обрезанный квадрат. Фамилия же есть только у
  // людей: у всех компаний в базе она пуста.
  const isPerson = Boolean(p.last_name && String(p.last_name).trim())

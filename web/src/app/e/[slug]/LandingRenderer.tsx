  ...
  return (
    ...
    >
      ...
      <style>{`
        ...
        /* Колонка шапки при сдвиге влево/вправо. Ширина приходит настройкой
           блока (--lp-hero-w). ⚠️ Только с планшета: на телефоне фон-обложка
           уходит наверх, а текст идёт под ней во всю ширину — узкая колонка
           там оставила бы половину экрана пустой. */
        @media (min-width: 768px) {
          .lp-hero-col { max-width: var(--lp-hero-w, 56%); }
        }
...
function BlockBody(props: any) {
  ...
  switch (block.kind) {
    ...
    case 'hero': {
      ...
      // ⚠️ При центре колонка занимает ВСЮ ширину (null) — так и задумано:
      // «Где стоит текст → По центру» это и есть «во всю полосу», а положение
      // текста и элементов внутри задаётся отдельно, настройкой «Строки внутри».
      const heroWidthPct = hAlign === 'center' ? null : (block.split_ratio || 56)
